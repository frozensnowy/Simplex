...
Events["intelevent_letsGo"] = 		--{{{"Sound_SetMuteActor('Fleet')", ""}, HW2_SubTitleEvent(Actor_FleetCommand, "$43770", 3)}, {HW2_Wait(0.5)}, {{"Universe_Fade(1, 1.5)", ""},}, {HW2_Wait(2.5)}, {{"Player_InstantDockAndParade(0, 'PlayerMothership', 0)", "", 0}, {"Player_InstantlyGatherAllResources(0)", ""}, {"Camera_FocusSobGroup('PlayerMothership', 1, 4000, 0)", ""},}, {{"Subtitle_Message('$3652', 2)", ""}, HW2_Wait(2)}, {HW2_Wait(0.5)}, {{"Universe_Fade(0, 1.5)", ""},}, {HW2_Wait(1.5)}, {HW2_SubTitleEvent(Actor_FleetCommand, "$43771", 3)}, {{"SobGroup_EnterHyperSpaceOffMap('Player_Ships0')", ""},}, {HW2_Wait(12)}, {{"Universe_Fade(1, 1.5)", ""},}, {HW2_Wait(1.5)}, {{"SobGroup_SwitchOwner('Dreadnaught', 3)", ""}, HW2_LocationCardEvent("$43881", 2), {"Sound_ExitIntelEvent()", ""}, {"EventPlaying = 0", ""},}, {HW2_Wait(0.5)}, {{"setMissionComplete(1)", ""},},}
    { 
        { 
            { "Sound_SetMuteActor('Fleet')", "", }, HW2_SubTitleEvent(Actor_FleetCommand, "$43770", 3), }, 
        { HW2_Wait(0.5), }, 
        { 
            { "Universe_Fade( 1, 1.5 )", "", }, 
        }, 
        ...
        { HW2_Wait(1.5), }, 
        { 
            { "SobGroup_SwitchOwner( 'Dreadnaught', 3 )", "", }, HW2_LocationCardEvent("$43881", 2), 
...
function Rule_Init()
  ...
  Player_SetPlayerNameAdvanced(4, "")
	...
	SobGroup_SetInvulnerability("Berth", 1)
	CPU_Enable(1, 0)
	CPU_Enable(2, 0)
	CPU_Enable(3, 0)
	CPU_Enable(4, 0)
	...
	SetAlliance(1, 3)
	SetAlliance(3, 1)
	SetAlliance(2, 3)		--avoid collision damage
	SetAlliance(3, 2)
...
function Rule_GetReactiveFleetRatio()
  ...
  Player_GrantResearchOption(1, "SP_KeeperDroneHealthHack")
  Player_GrantResearchOption(1, "SP_KeeperDroneFirepowerHack")
  Player_GrantResearchOption(1, "SP_KeeperDroneAccuracyHack")
...
function Rule_intelevent_NISDone()
	...
	SobGroup_SetInvulnerability("Keeper", 1)
...
function Rule_DebugCheats()
	local PlayerID_Keeper = SobGroup_OwnedBy("Keeper")
	local hp_threshold = 0.5
	local timer = Universe_GameTime()
	if KeeperHealthLocked == 0 then
		powerListStored[PlayerID_Keeper + 1] = LastpowerListMax[PlayerID_Keeper+1]	--fill energy
	end
	local damage_taken = 0
	if SobGroup_HealthPercentage("Keeper")<hp_threshold and SobGroup_HealthPercentage("Keeper")>0 then
		SobGroup_SetHealth("Keeper", hp_threshold)		--keep keeper alive
		damage_taken = damage_taken + 1
	end
	local bounty = (2-timer/1000)^3 + damage_taken*600
	if bounty > 0 then
...
function Rule_WatchForProbes()
	...
	SobGroup_SobGroupAdd("tempSobGroupTwo", "tempSobGroupOne")
...
function Rule_intelevent_letsGo()
	...
	if (intelevent_keeperTrappedISDONE == 1 or SobGroup_Empty("Keeper") == 1) then
...
function Rule_buildDreadnaughtAttackDroids()
	...
	SobGroup_SetHealth_UpgradeOnly(AttackDroid_DreadnaughtFreshlyBuilt, 1)
	SobGroup_Create("SP_TempSOB_DroidToLaunch")
	SobGroup_Clear("SP_TempSOB_DroidToLaunch")
	SobGroup_GetSobGroupDockedWithGroup("Keeper", "SP_TempSOB_DroidToLaunch")
	SobGroup_Launch("SP_TempSOB_DroidToLaunch", "Keeper")
	SobGroup_SetTactics("SP_TempSOB_DroidToLaunch", AggressiveTactics)
	SobGroup_SetHealth_UpgradeOnly("SP_TempSOB_DroidToLaunch", 1)
	SobGroup_SetAutoLaunch("Keeper", ShipHoldLaunch)
...
function Rule_keeperHide()
	SobGroup_Stop(SobGroup_OwnedBy("Keeper"), "Keeper")
	SP_SobGroup_EnterHyperSpaceOffMapAndRecaharge("Keeper")
...
function Rule_keeperLockHealthRemove()
	...
	SobGroup_SetInvulnerability("Keeper", 0)
...
function Rule_buildInhibitorAttackDroids()
	...
	SobGroup_SetHealth_UpgradeOnly(AttackDroid_InhibitorFreshlyBuilt, 1)
	SobGroup_Create("SP_TempSOB_DroidToLaunch")
	SobGroup_Clear("SP_TempSOB_DroidToLaunch")
	SobGroup_GetSobGroupDockedWithGroup("Keeper", "SP_TempSOB_DroidToLaunch")
	SobGroup_Launch("SP_TempSOB_DroidToLaunch", "Keeper")
