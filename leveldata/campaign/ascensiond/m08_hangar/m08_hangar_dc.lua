...
function Rule_GetReactiveFleetRatio()
	...
	if (ReactiveFleetRatio>.75) and (ReactiveFleetRatio<1) then
...
function Rule_intelevent_meetTheKeeper()
	...
	if (EventPlaying==0) then
		if (SobGroup_Empty("NISTriggerShips")==0) then
...
function Rule_WatchForProbes()
	...
	if (RemoveWatchForProbes==1) then
		...
		if (ProbeInPosition[volNumber]==0) then
			...
			Rule_AddInterval("Rule_WatchForProbes", .25);
			Rule_Remove("Rule_WatchForProbes");
		else
			if (volNumber==6) then
				volNumber=0;
...
function Rule_intelevent_probeInPosition()
	for i=1, 6 do
		if (EventPlaying==0) then
			if (ProbeInPosition[i]==1) then
				if (ProbeInPositionONCE[i]==0) then
					if (RemoveProbePingONCE1==0) then
						if (ProbeInPosition[1]==1) then
							EventPlaying=1;
							Sound_EnterIntelEvent();
					...
					if (RemoveProbePingONCE2==0) then
						if (ProbeInPosition[2]==1) then
							EventPlaying=1;
							Sound_EnterIntelEvent();
					...
					if (RemoveProbePingONCE3==0) then
						if (ProbeInPosition[3]==1) then
							EventPlaying=1;
							Sound_EnterIntelEvent();
					...
					if (RemoveProbePingONCE4==0) then
						if (ProbeInPosition[4]==1) then
							EventPlaying=1;
							Sound_EnterIntelEvent();
							Event_Start("intelevent_scansNegative");
					...
					if (RemoveProbePingONCE5==0) then
						if (ProbeInPosition[5]==1) then
							EventPlaying=1;
							Sound_EnterIntelEvent();
							Event_Start("intelevent_noDevicesFound");
					...
					if (RemoveProbePingONCE6==0) then
						if (ProbeInPosition[6]==1) then
...
function Rule_intelevent_inhibitorsEnRoute()
	if (EventPlaying~=0) or (Inhibitor1EnRoute~=1) or (Inhibitor1EnRouteONCE==0) then
		EventPlaying=1;
		Sound_EnterIntelEvent();
	...
	if (EventPlaying~=0) or (Inhibitor2EnRoute~=1) or (Inhibitor2EnRouteONCE==0) then
		EventPlaying=1;
		Sound_EnterIntelEvent();
	...
	if (EventPlaying~=0) or (Inhibitor3EnRoute~=1) or (Inhibitor3EnRouteONCE==0) then
		EventPlaying=1;
		Sound_EnterIntelEvent();
	...
	if (Inhibitor1EnRoute~=1) or (Inhibitor2EnRoute~=1) or (Inhibitor3EnRoute==1) then
...
function Rule_intelevent_inhibitorsInstalled()
	if (EventPlaying~=0) or (PlayInstalledEvent1~=1) or (Inhibitor1InstalledONCE==0) then
		if (Sensors_ManagerIsUp()==1) then
			Sensors_Toggle(0);
			Sensors_EnableToggle(0);
			EventPlaying=1;
	...
	if (EventPlaying~=0) or (PlayInstalledEvent2~=1) or (Inhibitor2InstalledONCE==0) then
		EventPlaying=1;
		Sound_EnterIntelEvent();
	...
	if (EventPlaying~=0) or (PlayInstalledEvent3~=1) or (Inhibitor3InstalledONCE==0) then
		EventPlaying=1;
		Sound_EnterIntelEvent();
	...
	if (Inhibitor1InstalledONCE~=1) or (Inhibitor2InstalledONCE~=1) or (Inhibitor3InstalledONCE==1) then
...
function Rule_intelevent_thirtySecondsToSelfDestruct()
	if (EventPlaying==0) then
		if (Timer_GetRemaining(0)<30) then
...
function Rule_keeperHealPhase1()
	...
	HealedKeeperHealth=(CurrentKeeperHealth + .01);
	SobGroup_SetHealth("Keeper", HealedKeeperHealth);
	Rule_AddInterval("Rule_keeperCheckHealthPhase1", .5);
...
function Rule_keeperCheckHealthPhase1()
	...
	if (SobGroup_HealthPercentage("Keeper")>.99) then
...
function Rule_keeperDecide()
	if (SobGroup_AreAllInHyperspace("Keeper")==1) then
		...
		if (KeeperGoGetTrapped==1) then
			...
			if (DesireToAttackFleet>2) or (Timer_GetRemaining("KeeperLastAttackTimer")<1) then
										...
										if (SobGroup_Empty("tempSobGroup")~=1) then
											...
											if (DesireToAttackFleet>2) or (Timer_GetRemaining("KeeperLastAttackTimer")<1) then
												...
												elseif (DesireToAttackFleet>2) or (Timer_GetRemaining("KeeperLastAttackTimer")<1) then
													...
													Rule_AddInterval("Rule_keeperAttack", 1);
													Rule_Remove("Rule_keeperDecide");
												elseif (CreateFirstInhibitorDroids==0) then
													if (InhibitorTrapHasStarted==1) then
														if (SobGroup_Empty("tempSobGroupOne")~=1) then
															...
															Rule_Remove("Rule_keeperDecide");
														elseif (CreateFirstInhibitorDroids==1) then
															if (SobGroup_Count("AttackDroid_Inhibitor")<8) then
																...
																Rule_AddInterval("Rule_keeperSpawnDroidsAtInhibitorTrap", 1);
																Rule_Remove("Rule_keeperDecide");
															if (SobGroup_Count("AttackDroid_Dreadnaught")<5) and (DreadnaughtInProximityOfMS==0) then
...
function Rule_keeperFIRSTTIMEAttackWatch()
	...
	elseif (SobGroup_HealthPercentage("Keeper")<.7) then
...
function Rule_keeperAttackWatch()
	Player_FillSobGroupInVolume("tempSobGroupOne", 0, "InhibitorTrapSphere");
	if (SobGroup_HealthPercentage("Keeper")<.7) then
	...
	elseif (KeeperGoGetTrapped==1) then
		...
		Timer_Start("KeeperLastAttackTimer", 170);
		Rule_Remove("Rule_keeperAttackWatch");
	elseif (Timer_GetRemaining("KeeperAttackTimer")<1) then
		if (SobGroup_Count("AttackDroid_Dreadnaught")<5) then
			if (DreadnaughtInProximityOfMS==0) then
				...
				Rule_Remove("Rule_keeperAttackWatch");
			elseif (Timer_GetRemaining("KeeperAttackTimer")<1) then
				if (CreateFirstInhibitorDroids==0) then
					if (InhibitorTrapHasStarted==1) then
						if (SobGroup_Empty("tempSobGroupOne")~=1) then
							CreateFirstInhibitorDroids=1;
							...
							Rule_Remove("Rule_keeperAttackWatch");
						elseif (Timer_GetRemaining("KeeperAttackTimer")<1) then
							if (CreateFirstInhibitorDroids==1) then
								if (SobGroup_Count("AttackDroid_Inhibitor")<8) then
...
function Rule_keeperHeal()
	...
	if (KeeperGoGetTrapped==1) then
		...
		HealedKeeperHealth=(CurrentKeeperHealth + .01);
		SobGroup_SetHealth("Keeper", HealedKeeperHealth);
		Rule_AddInterval("Rule_keeperCheckHealth", .5);
		Rule_Remove("Rule_keeperHeal");
	end
...
function Rule_StartKeeperSelfDestruct()
	...
	Rule_Remove("Rule_keeperLockHealth");
	Rule_AddInterval("Rule_DamageKeeper", .5);
...
function Rule_DamageKeeper()
	SobGroup_TakeDamage("Keeper", .0055);
...
Events={};
Events["intelevent_intro"]={{{"Rule_Add ('Rule_PlaySaveGameLocationCard')",
	""},
	HW2_Wait(1)},
	{{"g_save_id = g_save_id + 1",
	""},
	{"Campaign_QuickSaveNb( g_save_id, '$6431' )",
	""}},
	{HW2_Wait(2)},
	{HW2_LocationCardEvent("$43520", 6)},
	{{"Sound_EnableAllSpeech( 1 )",
	""},
	HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetCommand, "$43530", 3)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetCommand, "$43531", 2)},
	{HW2_Wait(1)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43532", 5)},
	{HW2_Letterbox(1),
	...
	""},
	HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43536", 4),
	{"pointer_powerSignature_id = HW2_CreateEventPointerSobGroup( 'Berth' )",
	""}},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43537", 8)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43534", 4),
	{"ping_powerSignature_id = Ping_AddSobGroup( ping_powerSignature, 'anomaly', 'Berth' )",
	""},
	...
	""}},
	{HW2_Wait(1)},
	{{"EventPointer_Remove(pointer_powerSignature_id)",
	""}},
	...
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_meetTheKeeper"]={{HW2_Wait(1)},
	{{"Ping_Remove( ping_powerSignature_id )",
	""},
	{"Objective_SetState( obj_prim_investigatePowerSignature_id, OS_Complete )",
	""},
	{"Camera_SetLetterboxStateNoUI(1, 3 )",
	""}},
	{HW2_Wait(3.5)},
	{{"g_currentNISPlaying_id = NISPlayFromGroup( 'nis/NIS08', 'Berth' )",
	""},
	{"Rule_Add ('Rule_intelevent_NISDone')",
	""}}};
Events["intelevent_retrieveTheDreadnaught"]={{HW2_Pause(1)},
	{HW2_Wait(2)},
	{{"Universe_EnableSkip(1)",
	""},
	{"Sensors_Toggle( 1 )",
	""}},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43610", 4),
	{"pointer_dreadnaught_id = HW2_CreateEventPointerSobGroup( 'Dreadnaught' )",
	""}},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43611", 4),
	{"ping_dreadnaught_id = Ping_AddSobGroup( ping_dreadnaught, 'anomaly', 'Dreadnaught' )",
	""},
	{"Ping_AddDescription(ping_dreadnaught_id, 0, '$43811')",
	""},
	...
	""}},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43612", 4)},
	{HW2_Wait(.5)},
	{{"EventPointer_Remove(pointer_dreadnaught_id)",
	""}},
	{{"Universe_EnableSkip(0)",
	""},
	HW2_Pause(0),
	...
	{{"g_save_id = g_save_id + 1",
	""},
	{"Campaign_QuickSaveNb( g_save_id, '$6432' )",
	""}},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	...
	{"EventPlaying = 0",
	""}}};
Events["intelevent_hyperspaceDisabled"]={{HW2_SubTitleEvent(Actor_FleetCommand, "$43605", 7)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetCommand, "$43606", 5)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_raisingTheDreadnaught"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43620", 3),
	{"Ping_Remove( ping_dreadnaught_id )",
	""}},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43621", 4)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_droidAnalysis"]={{HW2_Wait(3)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43615", 5)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43616", 7)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43617", 7)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_dreadnaughtMoverLost"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43630", 4)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43631", 5)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_dreadnaughtDocked"]={{{"print ('trying to play dreadnaught docked event')",
	""},
	{"Objective_SetState( obj_prim_retrieveDreadnaught_id, OS_Complete )",
	""},
	{"Ping_Remove( ping_dreadnaught_id )",
	""},
	HW2_SubTitleEvent(Actor_FleetIntel, "$43640", 2.5)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43641", 4)},
	{HW2_Wait(8)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43642", 3)},
	{HW2_Wait(1)},
	{{"Rule_Add ('Rule_PlaySaveGameLocationCard')",
	""},
	HW2_Wait(1)},
	{{"g_save_id = g_save_id + 1",
	""},
	{"Campaign_QuickSaveNb( g_save_id, '$6433' )",
	""}},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"intelevent_dreadnaughtDockedISDONE = 1",
	""},
	{"Rule_AddInterval ('Rule_launchDreadnaughtFromMS', 1)",
	""},
	{"EventPlaying = 0",
	""},
	{"Rule_AddInterval ('Rule_intelevent_cannotDefeatKeeper', 12)",
	""}}};
Events["intelevent_dreadnaughtAnalysis"]={{HW2_Wait(2)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43650", 5)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43651", 6)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43652", 5)},
	{HW2_Wait(.5)},
	{{"Sound_ExitIntelEvent()",
	""},
	{"EventPlaying = 0",
	""},
	{"intelevent_dreadnaughtAnalysisISDONE = 1",
	""},
	{"Rule_AddInterval ('Rule_intelevent_letsGo', 3)",
	""}}};
Events["intelevent_keeperAttacking"]={{{"Rule_Add ('Rule_PlayBattleMusic')",
	...
	""},
	HW2_SubTitleEvent(Actor_FleetIntel, "$43590", 4),
	{"pointer_keeper_id = HW2_CreateEventPointerSobGroup( 'Keeper' )",
	""}},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetCommand, "$43591", 5)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetCommand, "$43592", 6)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetCommand, "$43593", 4)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43594", 5),
	...
	""}},
	{HW2_Wait(.5)},
	{{"EventPointer_Remove(pointer_keeper_id)",
	""}},
	{{"Universe_EnableSkip(0)",
	""},
	HW2_Pause(0),
	...
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_cannotDefeatKeeper"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43700", 5)},
	{HW2_Wait(.5)},
	{HW2_Letterbox(1),
	{"Universe_EnableSkip(1)",
	""}},
	...
	""}},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43701", 5)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43702", 6),
	{"pointer_derelict1_id = HW2_CreateEventPointerSobGroup( 'InhibitorChunk1' )",
	""},
	{"pointer_derelict2_id = HW2_CreateEventPointerSobGroup( 'InhibitorChunk2' )",
	""},
	{"pointer_derelict3_id = HW2_CreateEventPointerSobGroup( 'InhibitorChunk3' )",
	""}},
	{HW2_Wait(1)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43709", 8)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43704", 8)},
	{{"EventPointer_Remove(pointer_derelict1_id)",
	""},
	{"EventPointer_Remove(pointer_derelict2_id)",
	""},
	{"EventPointer_Remove(pointer_derelict3_id)",
	""},
	HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43705", 8),
	{"ping_probeLocation_id1 = Ping_AddPoint( ping_probeLocation1, 'anomaly', 'ProbeLocation1' )",
	""},
	{"Ping_AddDescription(ping_probeLocation_id1, 0, '$43816')",
	""},
	{"ping_probeLocation_id2 = Ping_AddPoint( ping_probeLocation2, 'anomaly', 'ProbeLocation2' )",
	""},
	{"Ping_AddDescription(ping_probeLocation_id2, 0, '$43816')",
	""},
	{"ping_probeLocation_id3 = Ping_AddPoint( ping_probeLocation3, 'anomaly', 'ProbeLocation3' )",
	""},
	{"Ping_AddDescription(ping_probeLocation_id3, 0, '$43816')",
	""},
	{"ping_probeLocation_id4 = Ping_AddPoint( ping_probeLocation4, 'anomaly', 'ProbeLocation4' )",
	""},
	{"Ping_AddDescription(ping_probeLocation_id4, 0, '$43816')",
	""},
	{"ping_probeLocation_id5 = Ping_AddPoint( ping_probeLocation5, 'anomaly', 'ProbeLocation5' )",
	""},
	{"Ping_AddDescription(ping_probeLocation_id5, 0, '$43816')",
	""},
	{"ping_probeLocation_id6 = Ping_AddPoint( ping_probeLocation6, 'anomaly', 'ProbeLocation6' )",
	""},
	...
	""}}};
Events["intelevent_probeInPosition"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43720", 5)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_scansNegative"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43725", 5)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_noDevicesFound"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43726", 5)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_scanResultsNegative"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43727", 5)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_devicesLocated"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43721", 5),
	...
	{"EventPlaying = 0",
	""}}};
Events["intelevent_installInhibitors"]={{HW2_Letterbox(1),
	{"Universe_EnableSkip(1)",
	""}},
	{HW2_Wait(.5)},
	{HW2_Pause(1)},
	...
	""}},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43706", 5),
	{"pointer_inhibitor1_id = HW2_CreateEventPointerVolume('InhibitorSphere')",
	""}},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43708", 9),
	{"ping_inhibitor1_id = Ping_AddSobGroup( ping_inhibitor1, 'anomaly', 'HSInhibitor1' )",
	""},
	{"Ping_AddDescription(ping_inhibitor1_id, 0, '$43814')",
	""},
	...
	""}},
	{HW2_Wait(2)},
	{{"EventPointer_Remove(pointer_inhibitor1_id)",
	""}},
	{{"Universe_EnableSkip(0)",
	""},
	HW2_Pause(0),
	...
	{{"g_save_id = g_save_id + 1",
	""},
	{"Campaign_QuickSaveNb( g_save_id, '$6434' )",
	""}},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_inhibitor1EnRoute"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43710", 3)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_inhibitor2EnRoute"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43710", 3)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_inhibitor3EnRoute"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43710", 3)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_inhibitor1Installed"]={{{"Universe_EnableSkip(1)",
	""},
	HW2_Letterbox(1),
	HW2_Wait(4)},
	{{"Rule_Add ('Rule_FocusOnFirstInstalledInhibitor')",
	""}},
	{HW2_Wait(1)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43740", 3)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43742", 3)},
	{HW2_Wait(1)},
	{HW2_Letterbox(0),
	{"Universe_EnableSkip(0)",
	""},
	...
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_inhibitor2Installed"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43745", 3)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43742", 3)},
	{{"Sound_ExitIntelEvent()",
	""},
	...
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_inhibitor3Installed"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43750", 3)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43742", 3)},
	{{"Sound_ExitIntelEvent()",
	""},
	...
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_distortionFieldActive"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43756", 3),
	{"Objective_SetState( obj_prim_installInhibitors_id, OS_Complete )",
	""}},
	{{"Rule_Add ('Rule_PlaySaveGameLocationCard')",
	""},
	HW2_Wait(1)},
	{{"g_save_id = g_save_id +1",
	""},
	{"Campaign_QuickSaveNb( g_save_id, '$6435' )",
	""}},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_keeperTrapped"]={{HW2_Wait(1)},
	{HW2_Letterbox(1)},
	{HW2_Wait(3)},
	...
	""}},
	{HW2_Wait(7)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43578", 5)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetCommand, "$43579", 5)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43580", 5)},
	{HW2_Wait(.5)},
	{HW2_SubTitleEvent(Actor_FleetIntel, "$43582", 5)},
	{HW2_Wait(.5)},
	{{"intelevent_keeperTrappedISDONE = 1",
	""}},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_oneMinuteToSelfDestruct"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43583", 5)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_thirtySecondsToSelfDestruct"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43584", 5)},
	{{"Sound_ExitIntelEvent()",
	""},
	HW2_Wait(2)},
	{{"EventPlaying = 0",
	""}}};
Events["intelevent_letsGo"]={{{"Sound_SetMuteActor('Fleet')",
	""},
	HW2_SubTitleEvent(Actor_FleetCommand, "$43770", 3)},
	{HW2_Wait(.5)},
	{{"Universe_Fade( 1, 1.5 )",
	""}},
	{HW2_Wait(2.5)},
	...
	""}},
	{HW2_Wait(1.5)},
	{HW2_SubTitleEvent(Actor_FleetCommand, "$43771", 3)},
	...
	""}},
	{HW2_Wait(1.5)},
	{{"SobGroup_SwitchOwner( 'Dreadnaught', 3 )",
	""},
	HW2_LocationCardEvent("$43881", 2),
	{"Sound_ExitIntelEvent()",
	""},
	{"EventPlaying = 0",
	""}},
	...
	""}}};
Events["intelevent_mothershipDestroyed"]={{HW2_SubTitleEvent(Actor_FleetIntel, "$43880", 10)},
	{HW2_Wait(.5)},
