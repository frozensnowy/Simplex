...
function Debug_3Sek()
	...
	if Player_IsAlive(playerIndex) == 1 then
		local ship = "vgr_hyperspace_platform"..playerIndex		--"vgr_superbattleship"..playerIndex
		if SobGroup_Empty(ship) == 0 then
			if SobGroup_Selected(ship) == 1 then
				if 1==1 then
					for i=1,15,1 do
						print("<DEBUG>\t Switching "..ship.." from player "..playerIndex.." to player "..playerIndex..", time: "..Universe_GameTime())
						SobGroup_SetSwitchOwnerFlag(ship, 0)
						SobGroup_SwitchOwner(ship, playerIndex)
						SobGroup_MakeUntargeted(ship)
					end
				else
					local shiptable = ShipList_GetByRace(playerIndex)
					if next_ship_idx < getn(shiptable) then
						local shipname = shiptable[next_ship_idx].name
						print("<DEBUG>\t Creating "..shipname.." from "..ship..", time: "..Universe_GameTime())
						SobGroup_CreateShip(ship, shipname)
						--SobGroup_Launch_Smart(ship, "vgr_scout"..playerIndex)
						next_ship_idx = next_ship_idx + 1
					end
				end
			end
		end
