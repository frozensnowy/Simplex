...
--ark self defense
						if SobGroup_Count("hgn_ark"..playerIndex) >= 1 then
							if SobGroup_UnderAttack("hgn_ark"..playerIndex) == 1 then
								SobGroup_Create("ArkEnemySob"..playerIndex)
								SobGroup_Clear("ArkEnemySob"..playerIndex)
								SobGroup_GetAttackers("hgn_ark"..playerIndex, "ArkEnemySob"..playerIndex)
								SobGroup_Attack(playerIndex, "hgn_ark"..playerIndex, "ArkEnemySob"..playerIndex) 
	          	end
	          end
...
--hyperspace gate AI
						if SobGroup_Count("vgr_hyperspace_platform"..playerIndex) >= 1 then
							...
							local z = 0
							local connectome = {}	--table to avoid conflicting connection, will this solve the 00000030 crash?
			          ...
			          --form hyperspace link
				          elseif SobGroup_CanDoAbility("hyperspacePlatform_SOB"..z, AB_Move) == 0 and SobGroup_IsDoingAbility("hyperspacePlatform_SOB"..z, AB_Move) == 0 then
					        	...
					        	SobGroup_GetDistanceToSobGroup("hyperspacePlatform_SOB"..z, "hyperspacePlatform_SOB"..y) > 13000 then
					        		if Table_InsertUnique(connectome, z) == 1 and Table_InsertUnique(connectome, y) == 1 then 
  ...
  if Temp_GlobalVariable.AI_AugmentedSkills_Save == 7 and SobGroup_Count("SOB_AllDummyHSGates") >= 2 then
  	local numHSP = SobGroup_SplitGroupReference("SingleDummyHSGateToKill_SOB", "SOB_AllDummyHSGates", "AllShips", SobGroup_Count("SOB_AllDummyHSGates"), 0)
		for counter = 0, numHSP-1, 1 do
			if SobGroup_HealthPercentage("SingleDummyHSGateToKill_SOB"..counter) > 0 then
				SobGroup_Create("CheckDummyHSGatesProximity_SOB")
				SobGroup_FillProximitySobGroup("CheckDummyHSGatesProximity_SOB", "AllShips", "SingleDummyHSGateToKill_SOB"..counter, 8000)
				SobGroup_SobGroupSubstract("CheckDummyHSGatesProximity_SOB", "SOB_AllDummyHSGates")
				SobGroup_SobGroupSubstract("CheckDummyHSGatesProximity_SOB", "all_supporters")
				if SobGroup_Empty("CheckDummyHSGatesProximity_SOB") == 1 then
					debugger_printer("Killing useless gates, gate count: "..SobGroup_Count("SOB_AllDummyHSGates")..", year: "..Debug_year())
					SobGroup_SetHealth("SingleDummyHSGateToKill_SOB"..counter, 0)
				end
			end
		end
	end
			          					...
			          					SobGroup_CountCompare(jumper, "haxor_all_shipswithoutai") == 0 then
								          	...
								          	SobGroup_IsDoingAbility(jumper, AB_Parade) == 0 then
								          		...
								          		if SobGroup_OwnedBy(entrance) ~= playerIndex then
								          			if Table_InsertUnique(L4_cache.DummyHSGateFlags, counter) == 1 then
...
function AI_Survival()	--initiated from interface.lua
	...
	if Universe_GameTime() > 62 then
		...
		elseif Player_IsAlive(survival_data.WaveOwner) == 1 then	--begin survival waves
			...
			if survival_data.WaveTimer * survival_data.WaveFrequency > 100 then --reset the timer only after this interval
...
function AI_WaveSpawn(playerIndex)	--spawn a wave belonging to the player and let it attack other players
	...
	local shiptable = Player_GetShipTableByRace(playerIndex)
	local target_list = {"productioncapital", "smallcapitals", "juggernaughts", "utilities", "platforms", "frigates", "corvettes", "fighters", "shipwreck", "othersnoparade", "Player_Ships"}
	local exclude_name_patterns = {"station", "hgn_power", "hgn_combatbase", "fortress", "mothership", "shipyard", "carrier", "scaver", "vgr_am", "hgn_rts", "hgn_tanker", "sensor", "prayer", "hgn_artillerysentinel"}
	...
	if survival_wavelod == -1 then
		...
		if nf^2*LOD_multiplier < n_players then
			tinsert(exclude_classes, "frigate")
		end
		if np^2*LOD_multiplier < n_players then
			tinsert(exclude_classes, "platform")
		end
		if (nd+nbc)^2*LOD_multiplier < n_players then
			tinsert(exclude_classes, "armedcapital")
		end
	end
	...
	for y=1,2 do	--2 passes to build the candidate ship list, the second pass has looser criterion to ensure there is something added to the list
		for z, candicate in shiptable do
			...
			candicate.officer >= min_officer and candicate.officer <= max_officer then
				tinsert(candidate_table, candicate.name)
				shipfound = 1
			end
		end
	...
	if getn(candidate_table) > 0 then
		...
		for idx, target_type in target_list do
			...
			if SobGroup_Empty(target) == 0 then
				if (SobGroup_AreAllInRealSpace(target) == 1 and SobGroup_IsDocked(target) == 0) or idx == getn(target_list) then
					local radius = 4000
					...
					for n=1, CurrentWaveSize, 1 do
						...
						if 1==2 then	--SobGroup_SobGroupCompare(target, "productioncapital"..playerIndex) == 1	--debug mode
							...
							Table_InsertUnique(spawned_list, shipname)
						else	--normal spawn
							local theta = random_pseudo(-90,90)
							local beta = random_pseudo(0,360)
							coord[1] = pos[1] + radius*cos(theta)*cos(beta)
							...
							SobGroup_SpawnNewShipInSobGroup(playerIndex, shipname, "Wave", "SOB_Temp_Waves", "VOL_AISpawnWaves"..target..unique_serial)
							...
							SobGroup_CanDoAbility("SOB_Temp_Waves", AB_Attack) == 1 then
								if SobGroup_GetHardPointHealth("SOB_Temp_Waves", "experience") > 0 then
									SobGroup_SetHardPointHealth("SOB_Temp_Waves", "experience", max(0.1, 0.0010*LOD))
								end
			          ...
			          Ping_LabelVisible(megalitenoowner17, 1)
					      Ping_AddDescription(megalitenoowner17, 0, "Wave "..survival_data.WaveCounter)
								--SobGroup_SetTactics_Smart("SOB_Temp_Waves", AggressiveTactics)
								--SobGroup_Attack(playerIndex, "SOB_Temp_Waves", "enemies"..playerIndex)
								--if CPU_Exist_Smart(playerIndex) == 1 then
									--CPU_RemoveSobGroup(playerIndex, "SOB_Temp_Waves")
								--end
...
function AI_WaveAttack(playerIndex)
	...
	if SobGroup_Empty("SOB_Temp_Attacker") == 0 and CPU_Exist_Smart(playerIndex) == 1 then
		SobGroup_Create("CheckWaveTargets_SOB")
		SobGroup_SobGroupAdd("CheckWaveTargets_SOB", "enemies"..playerIndex)
		SobGroup_SobGroupSubstract("CheckWaveTargets_SOB", "SOB_AllDummyHSGates")
		SobGroup_SobGroupSubstract("CheckWaveTargets_SOB", "all_supporters")
		if SobGroup_Empty("CheckWaveTargets_SOB") == 0 and 
		SobGroup_IsDoingAbility("SOB_Temp_Attacker", AB_Attack) == 0 and 
		SobGroup_CanDoAbility("SOB_Temp_Attacker", AB_Attack) == 1 then
			CPU_RemoveSobGroup(playerIndex, "SOB_Temp_Attacker")
			--SobGroup_MoveToSobGroup(survivor, "SOB_Temp_Attacker", "CheckWaveTargets_SOB")
			SobGroup_Attack(playerIndex, "SOB_Temp_Attacker", "CheckWaveTargets_SOB")
		else
			CPU_AddSobGroup(playerIndex, "SOB_Temp_Attacker")
		end
		SobGroup_Clear("CheckWaveTargets_SOB")
	end
      	...
      	--encounter alert for human players
				if SobGroup_IsGroupNearybyExcludingSelf(survivor, "SoleSurvivor"..Universe_CurrentPlayer(), 5000+CPULODvalueList[playerIndexList]*1000) == 1 then
					if AreAllied(Universe_CurrentPlayer(), playerIndex)	== 0 then
	  				...
	  				if Universe_GameTime() - EncounterAnotherSurvivor[Universe_CurrentPlayer()+1][playerIndexList] > 100 then		--to prevent message spam
	  					SFXlist = {"SFX_cute_reptilian_greetings_01", "SFX_cute_reptilian_greetings_02", "SFX_cute_reptilian_greetings_03", "SFX_cute_reptilian_greetings_04", "SFX_cute_reptilian_greetings_05", }
...
function AI_GhostAttack(playerIndex)	--spawns ghost ships to attack player (called from megalith.lua)
	...
	local exclude_name_patterns = {"station", "fortress", "mothership", "shipyard", "carrier", "scaver", "vgr_am", "hgn_rts", "hgn_tanker", "hgn_artillerysentinel"}
	local exclude_classes = {"utility"}
