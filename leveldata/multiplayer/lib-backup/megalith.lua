...
Crate_ShipList_Capital = {"HWAT_VAY_CABattle", "HWAT_UNH_CAStandard", "HWAT_FRN_CAHeavy"}
	  		...
	  		if SobGroup_Empty("splitcrate"..i) == 0 then
					if SobGroup_AreAnyOfTheseTypes("splitcrate"..i, "meg_crate") == 1 and SobGroup_HealthPercentage("splitcrate"..i) > 0 then
						...
						if SobGroup_Count("ProximityOfCrateSOB") > 0 then
							...
							local outcome_list = {"ship", "ru", "en", "xp", "tech", "hp", "death"}
							if GameRulesName == "Simplex Sole Survivor" then
								outcome_list = {"ship", "ru", "en", "xp", "tech_ss", "hp", "up_health", "up_damage", "up_aim", "up_speed", "up_repair", "up_sensor", "up_nebula"}
							end
				        	...
				        	if SobGroup_AreAnyOfTheseTypes("Player_Ships"..claimer, GetSublistByPlayerRace(BuilderSpawnList, claimer).lost) == 0 then
										...
										SobGroup_SpawnNewShipInSobGroup(claimer, spawn_type, "ShipSpawned", "SingleShipSpawnedByCrateSOB", "VOL_CrateSpawningShipsTarget")
										spawn_completed = 1
		  						...
		  						while p < Universe_PlayerCount() do
									...
									(bounty_frigates >= 2*Universe_PlayerCount() and ship_table == Crate_ShipList_Frigate) then			--unit caps reached, just RUs
										...
										for h=0,iteration,1 do
											...
											SobGroup_SpawnNewShipInSobGroup(claimer, ship_table[shiptype], "ShipSpawned", "SingleShipSpawnedByCrateSOB", "VOL_CrateSpawningShipsTarget")
...
function EndgameCrisis()
	...
	local available_crises = {"meg_tmatmegaship","meg_bentusiexchange","meg_researchstation","meg_ghostship","meg_venator_destroyer","meg_allegiance_destroyer","meg_implacable_cruiser","meg_vengence_ssd","meg_eclipse_ssd","meg_class_iii_platform"}
		...
		if getn(L4_cache.EndgameCrisis) < crisis_count then	--every time a new megalith is spawned, its name is added to this list
			...
			if Table_ContainElement(L4_cache.EndgameCrisis, crisis) == 0 and interrupt == 0 then
				...
				if success == 1 then	--crisis spawned, alert the player in UI
					...
					local all_pictures = {"automated_dreadnought","enigmatic_fortress","gray_goo_ships","space_debris","asteroid_approaching_planet","fallen_empire_awakes","mining_station"}
				        ...
				        --cerca la Production Capital pi? vicina 
				                ...
				                SobGroup_FillShipsByType("temp", "carriers", "hgn_lightcarrier")             
          							SobGroup_FillSubstract("temp1", "productioncapitalallies"..playerIndex, "temp") 
												SobGroup_Clear("temp")
                        SobGroup_FillShipsByType("temp", "carriers", "kpr_carrier") 
								...
								for y=1,assemblyline,1 do
									if buildspeed>=random_pseudo(1,150) then
										...
										if getn(listname) > 0 then
											...
											SobGroup_SpawnNewShipInSobGroup(playerIndex, aircraft, "Airplane", "JetsToSpawn"..playerIndex, "VOL_AirCombatJetSpawnTarget")
...
function MegalithWreckCheck()	--used to switch megalith wrecks to player -1 so the "DestroyAll" game rule won't be violated
	...
	Update_AllShips()
	local megalith_wrecks = {"meg_bentus_wreck", "meg_dreadnaughtberth_wreck", "meg_tanis_wreck", "meg_balcoragate_wreck", "meg_gehenna_1ctm_wreck", "meg_gehenna_3ctm_wreck", "meg_progenitorpowermodule_wreck", "meg_foundry_wreck"}
				...
				elseif SobGroup_HealthPercentage(wreck) > 0.99 then
					...
					if shipmatched == 1 then
						...
						if can_build == 1 then
							...
							SobGroup_SpawnNewShipInSobGroup(playerIndex, shipname, "CurrentReborn", "CurrentReborn", "VOL_RebornSpawnTarget"..counter..volID)
