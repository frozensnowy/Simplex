function UI_SimplexKeyBinding()
	...
	UI_IsScreenActive("InGameMenu") == 0 then
		...
		if GetGameRubric() ~= GR_CAMPAIGN then
			...
			--UI_BindKeyEvent(KKEY, "KeyBinding_Diplomacy")	--doesn't work outside multiplayer
		end
	else
		...
		if GetGameRubric() ~= GR_CAMPAIGN then
			...
			--UI_UnBindKeyEvent(KKEY)
		end
	end
	UI_BindKeyEvent(ENTERKEY, "KeyBinding_Chat")
end
...
function KeyBinding_Diplomacy()
	UI_SwitchScreenOnAndOff("DiplomacyScreen")
  Rule_Remove("KeyBinding_Diplomacy")
end

function KeyBinding_Chat()
	UI_SwitchScreenOnAndOff("ChatScreen")
  Rule_Remove("KeyBinding_Chat")
end
  ...
  elseif ec_intro_count == 15 then
    ...
    if year <= 1 then   		    
	  ...
	  if GameRulesName == "Simplex Survival" then
	  	Rule_RandomInterval("AI_Survival", 3.262, 0)
	  end
  end 
...
function interfaceintro()   
	...
	--set text in the logo screen
  local rule_name = strupper(GameRulesName)
  local split_idx = strfind(rule_name, " ")
  --print("split_idx="..split_idx)
  local line1, line2, line3 = rule_name, "", "an idea by someone"
  if split_idx~=nil then
  	line1 = strsub(rule_name, 1, split_idx-1) --don't include the space at the end
  	line2 = strsub(rule_name, split_idx+1)
  	--print("line1="..line1..", line2="..line2)
  end
  UI_SetTextLabelText("ComplexLogo", "line1", line1);
  UI_SetTextLabelText("ComplexLogo", "line2", line2);
  UI_SetTextLabelText("ComplexLogo", "line3", line3);
    ...
    if Player_IsAlive(playerIndex) == 1 then
      if Player_HasShipWithBuildQueue(playerIndex) == 1 then   
			  if Player_GetRace(playerIndex) == Race_Hiigaran then
					...
					elseif SobGroup_Empty("Player_Ships"..playerIndex) == 0 then
						Camera_FocusSobGroupWithBuffer("Player_Ships"..Universe_CurrentPlayer(), 10000, 0, 29)
					end  
				elseif Player_GetRace(playerIndex) == Race_Vaygr then
...
function cleaninterface()
  ...
  UI_SetElementSize("NewTaskbar", "medal1", 9, 7)
  UI_SetElementSize("NewTaskbar", "medal2", 9, 7)
  UI_SetElementSize("NewTaskbar", "medal3", 9, 7)
  UI_SetElementSize("NewTaskbar", "medal4", 9, 7)
  UI_SetElementSize("NewTaskbar", "medal5", 9, 7)
  ...
  UI_SetElementSize("NewTaskbar", "fusiontitan", 0, 0); 
  UI_SetElementSize("NewTaskbar", "monster", 0, 0);  
  UI_SetElementSize("NewTaskbar", "freacker", 0, 0);	
  UI_SetTextLabelText("NewTaskbar", "experience_title", "");
  UI_SetTextLabelText("NewTaskbar", "experience", "");
...
function selected()
	...
	--print("selected -- Start, Selected_Save = "..Temp_GlobalVariable.Selected_Save)
  local playerIndex = Universe_CurrentPlayer()    
  ...
  en = 0
	...
	--Orders
	        ...
	        orders("splitmotherships"..i, "dock",0,0)
		      ...
		      orders("splitmotherships"..i, "tactical",0,0)
	...
	---viper for AI 	
	      	...
	      	elseif ((Player_HasResearch(SobGroup_OwnedBy("splitmotherships"..i), "isai" ) == 1) and
	...
	--Orders        	
	      	...
	      	if SobGroup_GetHardPointHealth("splitmotherships"..i, "rad_general_help") > 0 then
	...
	---selection
	        ...
	        DriveMode_DisplayButtonForShip("splitmotherships"..i)
		      ...
		      ---race for taskbar	    
		      UI_DisplayRace(SobGroup_OwnedBy("splitmotherships"..i))
		      if SobGroup_AreAnyOfTheseTypes("splitmotherships"..i, "hgn_mothership") == 1 then    
	...
	---Weapons set		    	    
	          ...
	          if SobGroup_GetHardPointHealth("splitmotherships"..i, "c") > 0 and SobGroup_OwnedBy("splitmotherships"..i) == playerIndex then  
				  ...
				  UI_DisplayExperience(SobGroup_OwnedBy("splitmotherships"..i), exp1)
	...
	--gestione del danno		       	     
	          ...
	          DriveMode_DisplayButtonForShip("splitresearchstations"..i)
			      UI_SetTextLabelText("NewTaskbar", "unitactualspeed", ""..floor(sqrt(SobGroup_GetActualSpeed("splitresearchstations"..i))));
				    ...
				    ---race for taskbar	    
				    UI_DisplayRace(SobGroup_OwnedBy("splitresearchstations"..i))
			      ...
			      en = SobGroup_GetHardPointHealth("splitresearchstations"..i, "en")		 			  		  		  		  	        
				    ...
				    UI_DisplayExperience(SobGroup_OwnedBy("splitresearchstations"..i), exp1)
			...
			--gestione del danno		       	     
	          ...
	          DriveMode_DisplayButtonForShip("splitweaponstations"..i)
			      UI_SetTextLabelText("NewTaskbar", "unitactualspeed", ""..floor(sqrt(SobGroup_GetActualSpeed("splitweaponstations"..i))));
				    ---race for taskbar	    
				    UI_DisplayRace(SobGroup_OwnedBy("splitweaponstations"..i))
			      ...
			      en = SobGroup_GetHardPointHealth("splitweaponstations"..i, "en")		 			  		  		  		  	        
				    ...
				    UI_DisplayExperience(SobGroup_OwnedBy("splitweaponstations"..i), exp1)
	      	...
	      	elseif ((Player_HasResearch(SobGroup_OwnedBy("splitweaponstations"..i), "isai" ) == 1) and
	      	   (SobGroup_FillProximitySobGroup ("temp"..SobGroup_OwnedBy("splitweaponstations"..i), "smallcapitalenemies"..SobGroup_OwnedBy("splitweaponstations"..i), "splitweaponstations"..i, 12500) == 1) and
						 (SobGroup_IsDoingAbility("splitweaponstations"..i, abilityList[2]) == 1) and
						 (SobGroup_UnderAttack("splitweaponstations"..i) == 1) and
	...
	--energy consume (sobgroup, multiplier, capacit?costruzione: 1 ship or subs, 2 both, 0 none)
	        ...
	        if SobGroup_GetHardPointHealth("splitpowerstations"..i, "powerup_power") > 0 then
	          ...
	          DriveMode_DisplayButtonForShip("splitpowerstations"..i)
			      ...
			      ---race for taskbar	    
			      UI_DisplayRace(SobGroup_OwnedBy("splitpowerstations"..i))
			      ...
			      en = SobGroup_GetHardPointHealth("splitpowerstations"..i, "en")		 			  		  		  		  	        
				    ...
				    UI_DisplayExperience(SobGroup_OwnedBy("splitpowerstations"..i), exp1)
	...
	--costruisce moduli rank, lo fa una volta sola se year < 1, usata nel caso si parta con un grado diverso da ensign	      
			...
			--gestione del danno		         	    
	          ...
	          DriveMode_DisplayButtonForShip("splitcrewstations"..i)
			      ...
			      ---race for taskbar	    
			      UI_DisplayRace(SobGroup_OwnedBy("splitcrewstations"..i))
			      ...
			      elseif gradoList[SobGroup_OwnedBy("splitcrewstations"..i) + 1] == "FLEET ADMIRAL" then 
				    ...
				    exp1 = SobGroup_GetHardPointHealth("splitcrewstations"..i, "experience") * 10000			      
					  ...
					  UI_DisplayExperience(SobGroup_OwnedBy("splitcrewstations"..i), exp1)
		...
		--Sun Damage
							...
							if SobGroup_Selected_Smart("splitothers"..pi .. tostring(i)) == 1 then
							  cleaninterface()	
								...
								---race for taskbar	    
								UI_DisplayRace(SobGroup_OwnedBy("splitothers"..pi .. tostring(i)))
		          ...
		          while SobGroup_Count("temp") == 0 and distance < 50000 do				                        						     
				        if SobGroup_FillProximitySobGroup ("temp", "hgn_crewstation"..SobGroup_OwnedBy("splitevacs"..pi .. tostring(i)), "splitevacs"..pi .. tostring(i), distance) == 1 then  							        
				          if SobGroup_CanDoAbility("temp", AB_AcceptDocking) == 1 then							          
						        SobGroup_DockSobGroupAndStayDocked_Smart("splitevacs"..pi .. tostring(i), "temp")
						        --break
						      else
						        SobGroup_Clear("temp")   
						      end  
						...
						if SobGroup_Selected_Smart("splitevacs"..pi .. tostring(i)) == 1 then		
						  ...
						  ---race for taskbar	    
						  UI_DisplayRace(SobGroup_OwnedBy("splitevacs"..pi .. tostring(i)))
					      ...
					      UI_SetTextLabelText("NewTaskbar", "Sfusion", ""..ff.."/"..fusionList[SobGroup_OwnedBy("splitmobilerefineries"..i) + 1]);
							...
							if SobGroup_AreAnyOfTheseTypes("splitmobilerefineries"..i, "rad_resourcecontroller") == 1 then
							  ...
							  if exp1 > 9999 then
								...
								UI_DisplayExperience(SobGroup_OwnedBy("splitmobilerefineries"..i), exp1)
							end
							---race for taskbar	    
							UI_DisplayRace(SobGroup_OwnedBy("splitmobilerefineries"..i))
	  ...
	  if (hwsaving == 9 and nrc >= 35) or nrc > 0 then		--nrc < 35
		...
		--AI salvage bug fix, very important!!! 
	            ...
	            DriveMode_DisplayButtonForShip("splitresourcecollectors"..i)
							...
							---race for taskbar	    
							UI_DisplayRace(SobGroup_OwnedBy("splitresourcecollectors"..i))
						...
						if SobGroup_IsDoingAbility("splitcontainers"..i, AB_Parade) == 1 and sqrt(SobGroup_GetEquivalentSpeed("splitcontainers"..i)) < 10 then
							if SobGroup_Empty("traderallies"..SobGroup_OwnedBy("splitcontainers"..i)) == 0 and SobGroup_Empty("trader"..SobGroup_OwnedBy("splitcontainers"..i)) == 0 then
								if SobGroup_Count("traderallies"..SobGroup_OwnedBy("splitcontainers"..i)) > SobGroup_Count("trader"..SobGroup_OwnedBy("splitcontainers"..i)) then
									SobGroup_DockSobGroup_Smart("splitcontainers"..i, "traderallies"..SobGroup_OwnedBy("splitcontainers"..i))
								end
							end
						end
					end
	          ...
	          if SobGroup_IsDocked("splitcontainers"..i) == 0 then 	
	           	if SobGroup_GetEquivalentSpeed("splitcontainers"..i) >= 16000 then
					...
					if SobGroup_Selected_Smart("splitcontainers"..i) == 1 then
						...
						---race for taskbar	    
						UI_DisplayRace(SobGroup_OwnedBy("splitcontainers"..i))
	  ...
	  if (hwsaving == 2 and nmc >= 15) or nmc > 0 then			--nmc < 15
		...
		--Sun Damage
	            ...
	            DriveMode_DisplayButtonForShip("splitminecontainers"..i)
							...
							---race for taskbar	 
							UI_DisplayRace(SobGroup_OwnedBy("splitminecontainers"..i))
		...
		--if hwsaving == 5 then
			if SobGroup_Empty("megalithnoowner") == 0 then	  
				if megalithnoownercount == 0 then  
					megalithnoownercount = SobGroup_SplitGroupReference("megalithnoowner", "megalithnoowner", "splitter", SobGroup_Count("megalithnoowner")) 	    
				end
			...
			--crates
			if CrateCount > 0 and Universe_GameTime() > 20 then
				for i = 0, CrateCount-1, 1 do  
					if SobGroup_Empty("splitcrate"..i) == 0 then	 	
						if SobGroup_Selected_Smart("splitcrate"..i) == 1 then			      
						  cleaninterface()	
	            interfacecleaned=interfacecleaned+2
	            DriveMode_DisplayButtonForShip("splitcrate"..i)
							UI_SetTextLabelText("NewTaskbar", "unitrace", "None");
							UI_SetTextLabelText("NewTaskbar", "unitactualspeed", ""..floor(sqrt(SobGroup_GetActualSpeed("splitcrate"..i))));
							--break
						end
					end
				end
			...
			if MaxCount[4] >= 0 and Universe_GameTime() > 20 then
				for i = 0,MaxCount[4],1 do  
					if SobGroup_Empty("megalite"..i) == 0 then
						if SobGroup_AreAnyOfTheseTypes("megalite"..i, "meg_headshotasteroid, meg_carrier_huge, meg_bentus, meg_researchstation, meg_bentusiexchange") == 1 then
			...
			if MaxCount[5] >= 0 and Universe_GameTime() > 20 then
	            ...
	            DriveMode_DisplayButtonForShip("planet"..i)
							...
							for p, iCount in planetPopulationList do
								if SobGroup_AreAnyOfTheseTypes("planet"..i, "hgn_"..iCount.planet) == 1 or 
								(iCount.planet == "planet_earth" and SobGroup_AreAnyOfTheseTypes("planet"..i, "hgn_planet_kharak, hgn_planet_kharak_burned, hgn_planet_hiigara, hgn_planet_earth_aerosol") == 1) or 	--these newly added planets share the same data entry as earth
								(iCount.planet == "planet_moon" and SobGroup_AreAnyOfTheseTypes("planet"..i, "hgn_planet_angelmoon") == 1) then
	  ...
	  if (hwsaving == 2 and nj >= 12) or nj > 0 then		--nj < 12
						...
						hwsaving == 2 then		--docking needs some time, so don't hurry, otherwise you'll throw too many boosters at a time 
							if SobGroup_Count("JuggernaughtBoostersDocked") < booster_slots then
								...
								elseif SobGroup_GetHardPointHealth("splitjuggernaughts"..i, "jbooster") >= 0.2 then	--all boosters are installed at once in the first place
									...
									SobGroup_DockSobGroupAndStayDocked_Smart("JuggernaughtBoosterSpawnSOB_"..SobGroup_OwnedBy("splitjuggernaughts"..i), "splitjuggernaughts"..i)
		...
		--Sun Damage
	            ...
	            DriveMode_DisplayButtonForShip("splitjuggernaughts"..i)
							...
							---race for taskbar	    
							UI_DisplayRace(SobGroup_OwnedBy("splitjuggernaughts"..i))
						  ...
						  if exp1 > 9999 then
							...
							UI_DisplayExperience(SobGroup_OwnedBy("splitjuggernaughts"..i), exp1)
	  ...
	  if (hwsaving == 6 and nnb >= 12) or nnb > 0 then		--nnb < 12
		...
		--Sun Damage
	            ...
	            DriveMode_DisplayButtonForShip("splitnuclearbombs"..i)
							...
							---race for taskbar	    
							UI_DisplayRace(SobGroup_OwnedBy("splitnuclearbombs"..i))
  ...
  if Temp_GlobalVariable.Selected_Save == 3 then
	---Destroyer
	  if (hwsaving == 0 and nd >= 20) or nd > 0 then		--nd < 20
		...
		--special weapons
			      	...
			      	elseif ((Player_HasResearch(SobGroup_OwnedBy("splitdestroyers"..i), "isai" ) == 1) and
		...
		--Energy Consume
	            ...
	            DriveMode_DisplayButtonForShip("splitdestroyers"..i)
							...
							---race for taskbar	    
							UI_DisplayRace(SobGroup_OwnedBy("splitdestroyers"..i))
	
							exp1 = SobGroup_GetHardPointHealth("splitdestroyers"..i, "experience") * 10000			 		  		  		   	   	        
						  if exp1 > 9999 then
						  	exp1 = 9999
						  end
							UI_DisplayExperience(SobGroup_OwnedBy("splitdestroyers"..i), exp1)
				   			...
				   			elseif SobGroup_AreAnyOfTheseTypes("splitdestroyers"..i, "vgr_pulsedestroyer") == 1 then
				   				UI_SetElementSize("NewTaskbar", "subsystems", 160, 24);
									...
									if Player_HasResearch(SobGroup_OwnedBy("splitdestroyers"..i), "MinePulseDestroyer") == 1 then
										...
										UI_SetTextLabelText("NewTaskbar", "fusionblastertext", "Laser");
									end	
	...
	--Orders
		      ...
		      elseif SobGroup_AreAnyOfTheseTypes("splitbattlecruisers"..i, "hgn_spacecarrier") == 1 then
		        --Orders
		        orders("splitbattlecruisers"..i, "clear",0,0)   
	        	orders("splitbattlecruisers"..i, "parade",0,0)
	        	orders("splitbattlecruisers"..i, "dock",0,0)
	        	orders("splitbattlecruisers"..i, "evacuation",10,1)	
	        	orders("splitbattlecruisers"..i, "guard",0,0)
	...
	--Orders                   
			      orders("splitbattlecruisers"..i, "guard",0,0)
			      orders("splitbattlecruisers"..i, "destroyallguns",0,0)
			    elseif SobGroup_AreAnyOfTheseTypes("splitbattlecruisers"..i, "vgr_battleship") == 1 then
	--Orders                   
			      orders("splitbattlecruisers"..i, "guard",0,0)
			    elseif SobGroup_AreAnyOfTheseTypes("splitbattlecruisers"..i, "vgr_missledreadnaught") == 1 then
						if ((CPU_Exist_Smart(SobGroup_OwnedBy("splitbattlecruisers"..i)) == 1) and
						   (SobGroup_IsDoingAbility("splitbattlecruisers"..i, abilityList[2]) == 1) and 
		      	   (SobGroup_FillProximitySobGroup ("temp"..SobGroup_OwnedBy("splitbattlecruisers"..i), "smallcapitalenemies"..SobGroup_OwnedBy("splitbattlecruisers"..i), "splitbattlecruisers"..i, 12500) == 1) and
		      	   (Player_GetNumberOfSquadronsOfTypeAwakeOrSleeping(SobGroup_OwnedBy("splitbattlecruisers"..i), "vgr_cruse1") < 1) and
	...
	--Orders                   
	        ...
	        elseif SobGroup_AreAnyOfTheseTypes("splitbattlecruisers"..i, "rad_armsdealer") == 1 then
	        	...
	        	if ((Player_HasResearch(SobGroup_OwnedBy("splitbattlecruisers"..i), "isai" ) == 1) and
		      	  ...
		      	  SobGroup_CreateShip("splitbattlecruisers"..i, "vgr_cruse1")						
		      	  Player_SetRU(SobGroup_OwnedBy("splitbattlecruisers"..i), Player_GetRU(SobGroup_OwnedBy("splitbattlecruisers"..i)) - 750)
		      	elseif ((Player_HasResearch(SobGroup_OwnedBy("splitbattlecruisers"..i), "isai" ) == 1) and
		      	   (SobGroup_FillProximitySobGroup ("temp"..SobGroup_OwnedBy("splitbattlecruisers"..i), "smallcapitalenemies"..SobGroup_OwnedBy("splitbattlecruisers"..i), "splitbattlecruisers"..i, 12500) == 1) and
							 (SobGroup_IsDoingAbility("splitbattlecruisers"..i, abilityList[2]) == 1) and
		...
		--avatar explosion damage funcion
			...
			--Switch phase cannon power for dreadnaughtB
	          ...
	          DriveMode_DisplayButtonForShip("splitbattlecruisers"..i)
			      ...
			      ---race for taskbar	    
			      UI_DisplayRace(SobGroup_OwnedBy("splitbattlecruisers"..i))
	
			      exp1 = SobGroup_GetHardPointHealth("splitbattlecruisers"..i, "experience") * 10000			 		  		  		   	   	        
					  if exp1 > 9999 then
					  	exp1 = 9999
					  end
				    UI_DisplayExperience(SobGroup_OwnedBy("splitbattlecruisers"..i), exp1)
	          ...
	          en = SobGroup_GetHardPointHealth("splitbattlecruisers"..i, "en")		 			  		  		  		  	        
				    UI_SetTextLabelText("NewTaskbar", "energy", ""..floor((en-0.5)*1000));							
					  ...
					  elseif SobGroup_AreAnyOfTheseTypes("splitbattlecruisers"..i, "vgr_battleship") == 1 then
				    	UI_SetElementSize("NewTaskbar", "subsystem11", 0, 0);
				    	UI_SetElementSize("NewTaskbar", "subsystem12", 0, 0);
					  	if Player_HasResearch(SobGroup_OwnedBy("splitbattlecruisers"..i), "ImprovedTitanBeam") == 1 then
								UI_SetElementVisible("NewTaskbar", "abilities1", 1)
								UI_SetElementSize("NewTaskbar", "fusiontitan", 66, 15);
								UI_SetTextLabelText("NewTaskbar", "fusiontitantext", "Shaker");
							end
					  elseif SobGroup_AreAnyOfTheseTypes("splitbattlecruisers"..i, "vgr_missledreadnaught") == 1 then
				    	UI_SetElementSize("NewTaskbar", "subsystem10", 0, 0);
				    	UI_SetElementSize("NewTaskbar", "subsystem11", 0, 0);
			      ...
			      elseif SobGroup_AreAnyOfTheseTypes("splitbattlecruisers"..i, "kpr_reaver") == 1 then    
			        ...
			        if docked_gen > 2 then
			        	docked_gen = 2
						  ...
						  UI_SetTextLabelText("NewTaskbar", "DockedPowerGenerators", ""..docked_gen)
			      elseif SobGroup_AreAnyOfTheseTypes("splitbattlecruisers"..i, "HWAT_UNH_CAStandard, HWAT_VAY_CABattle, HWAT_FRN_CAHeavy, vgr_worldcannon") == 1 then    
			        UI_SetElementSize("NewTaskbar", "subsystems", 0, 0); 
	        ...
	        elseif hwsaving == 6 and SobGroup_Empty("star_sol") == 0 then
	          sundamageinterface = SobGroup_TakeDamageSol("splitshipyards"..i,0.6,Temp_GlobalVariable.Selected_NextInterval)				
	        	...
	        	orders("splitshipyards"..i, "evacuation",40,3)        	           
		      	...
		      	orders("splitshipyards"..i, "tactical",0,0)
	...
	--costruisca produzione cap alternate se sy ha prod capital, serve per ricerca/costruzione di bc e vortex (req. adv. cap ship facility)
	          ...
	          DriveMode_DisplayButtonForShip("splitshipyards"..i)
			      ---race for taskbar		      
			      UI_DisplayRace(SobGroup_OwnedBy("splitshipyards"..i))
	          ...
	          UI_SetTextLabelText("NewTaskbar", "unitactualspeed", ""..floor(sqrt(SobGroup_GetActualSpeed("splitshipyards"..i))));           
				    exp1 = SobGroup_GetHardPointHealth("splitshipyards"..i, "experience") * 10000			 		  		  		   	   	        
					  ...
					  UI_DisplayExperience(SobGroup_OwnedBy("splitshipyards"..i), exp1)
	...
	--Orders
	          orders("splitcommandfortresses"..i, "clear",0,0)   
	        	...
	        	orders("splitcommandfortresses"..i, "tactical",0,0)
	--energy consume (sobgroup, multiplier, capacit?costruzione: 1 ship or subs, 2 both, 0 none) 
		        if hwsaving == 6 then        
	...
	--cruise missile for AI
				  ...
				  elseif SobGroup_AreAnyOfTheseTypes("splitcommandfortresses"..i, "vgr_superbattleship") == 1 then
					--Orders		      
	        	orders("splitcommandfortresses"..i, "parade",0,0)
			      orders("splitcommandfortresses"..i, "guard",0,0)
	--energy consume (sobgroup, multiplier, capacit?costruzione: 1 ship or subs, 2 both, 0 none) 
	          ...
	          DriveMode_DisplayButtonForShip("splitcommandfortresses"..i)
				    ...
				    ---race for taskbar	    
				    UI_DisplayRace(SobGroup_OwnedBy("splitcommandfortresses"..i))
	              
				    exp1 = SobGroup_GetHardPointHealth("splitcommandfortresses"..i, "experience") * 10000			 		  		  		   	   	        
					  if exp1 > 9999 then
					  	exp1 = 9999
					  end
					  UI_DisplayExperience(SobGroup_OwnedBy("splitcommandfortresses"..i), exp1)
	          ...
	          en = SobGroup_GetHardPointHealth("splitcommandfortresses"..i, "en")		 			  		  		  		  	        
				    UI_SetTextLabelText("NewTaskbar", "energy", ""..floor((en-0.5)*1000));							
					  ...
					  elseif SobGroup_AreAnyOfTheseTypes("splitcommandfortresses"..i, "vgr_superbattleship") == 1 then
					  	UI_SetElementSize("NewTaskbar", "subsystems", 210, 48);
					  	if Player_HasResearch(SobGroup_OwnedBy("splitcommandfortresses"..i), "ImprovedTitanBeam") == 1 then
								UI_SetElementVisible("NewTaskbar", "abilities1", 1)
								UI_SetElementSize("NewTaskbar", "fusiontitan", 66, 15);					      
								UI_SetTextLabelText("NewTaskbar", "fusiontitantext", "Shaker");
							end
	...
	--energy consume (sobgroup, multiplier, capacit?costruzione: 1 ship or subs, 2 both, 0 none)  
	        if hwsaving == 7 then       
		      ...
		      elseif SobGroup_AreAnyOfTheseTypes("splitcarriers"..i, "hgn_carrier") == 1 then	
	        	...
	        	orders("splitcarriers"..i, "guard",0,0)
	        	orders("splitcarriers"..i, "tactical",0,0)
	...
	--cruise missile for AI
	        ...
	        elseif SobGroup_AreAnyOfTheseTypes("splitcarriers"..i, "kpr_carrier") == 1 then
	          ...
	          DriveMode_DisplayButtonForShip("splitcarriers"..i)
			      ...
			      ---race for taskbar	    
			      UI_DisplayRace(SobGroup_OwnedBy("splitcarriers"..i))
	   
				    exp1 = SobGroup_GetHardPointHealth("splitcarriers"..i, "experience") * 10000			 		  		  		   	   	        
					  if exp1 > 9999 then
					  	exp1 = 9999
					  end
				    UI_DisplayExperience(SobGroup_OwnedBy("splitcarriers"..i), exp1)
  ...
  if Temp_GlobalVariable.Selected_Save == 6 then
	...
	--energy consume (sobgroup, multiplier, capacit?costruzione: 1 ship or subs, 2 both, 0 none) 
	        ...
	        elseif hwsaving == 9 and SobGroup_Empty("star_sol") == 0 then
					...
					if SobGroup_AreAnyOfTheseTypes("splithyperspacestations"..i, "hgn_hyperspacestation") == 1 then
	          ...
	          DriveMode_DisplayButtonForShip("splithyperspacestations"..i)
			      ...
			      ---race for taskbar	    
			      UI_DisplayRace(SobGroup_OwnedBy("splithyperspacestations"..i))
			      ...
			      UI_SetElementSize("NewTaskbar", "subsystems", 210, 24);
				    ...
				    UI_DisplayExperience(SobGroup_OwnedBy("splithyperspacestations"..i), exp1)
	        ...
	        if SobGroup_Empty("star_sol") == 0 and hwsaving == 7 then        
		      ...
		      if SobGroup_AreAnyOfTheseTypes("splitplatforms"..i, "hgn_massiveturret") == 0 then
										...
										if SobGroup_Empty("VgrMobileInhibitorComponentDocked") == 1 then		--no inhibitor installed
											...
											SobGroup_IsDoingAbility("splitplatforms"..i, AB_Dock) == 0 then
												...
												if SobGroup_IsDocked("VgrMobileInhibitorSpawnSOB_"..SobGroup_OwnedBy("splitplatforms"..i)) == 0 then
													SobGroup_DockSobGroupAndStayDocked_Smart("VgrMobileInhibitorSpawnSOB_"..SobGroup_OwnedBy("splitplatforms"..i), "splitplatforms"..i)
												end
	            ...
	            DriveMode_DisplayButtonForShip("splitplatforms"..i)
				      ...
				      ---race for taskbar	    
				      UI_DisplayRace(SobGroup_OwnedBy("splitplatforms"..i))
				      ...
				      if SobGroup_AreAnyOfTheseTypes("splitplatforms"..i, "hgn_defenderdrone, hgn_gunturret, hgn_ionturret, hgn_missileturret, hgn_pulsarturret, hgn_artillerysentinel, hgn_sentinel, hgn_ionsentinel, hgn_rts, hgn_defensefieldturret, vgr_cloakplatform, vgr_weaponplatform_gun, vgr_weaponplatform_lance, vgr_weaponplatform_missile, kad_snail, rad_guntrap, rad_hsbouey, kpr_sentinel, kpr_marauder, kpr_meddler, kpr_dronestruct") == 1 then
						    ...
						    UI_DisplayExperience(SobGroup_OwnedBy("splitplatforms"..i), exp1)
				      end
				      --fuel
	...
	---Ghosts
		if nghost > 0 then
		  for i = 0,nghost-1,1 do
		    if SobGroup_Empty("splitghosts"..i) == 0 then
		    	
		    	if SobGroup_InWorldBoundForKillingOnly("splitghosts"..i, 0) == 0 then
						SobGroup_SetHealth("splitghosts"..i, 0)
					end
					
					if SobGroup_Selected_Smart("splitghosts"..i) == 1 then
					  cleaninterface()
	          interfacecleaned=interfacecleaned+1
	          DriveMode_DisplayButtonForShip("splitghosts"..i)
			      UI_SetTextLabelText("NewTaskbar", "unitactualspeed", ""..floor(sqrt(SobGroup_GetActualSpeed("splitghosts"..i))));
			      ---race for taskbar
			      UI_DisplayRace(SobGroup_OwnedBy("splitghosts"..i))
				    UI_SetTextLabelText("NewTaskbar", "energy", "???");
			      UI_SetElementSize("NewTaskbar", "subsystems", 0, 0);
				    UI_SetTextLabelText("NewTaskbar", "experience", "???");
					  UI_SetTextLabelText("NewTaskbar", "efficency", "???");
		        --break
			    end
		    end
		  end
		  ...
		  if (hwsaving == 1 and nfi[playervar] >= 10) or nfi[playervar] > 0 then		--nfi[playerIndexList] < 10
			...
			--Sun Damage
		            ...
		            DriveMode_DisplayButtonForShip("splitfighters"..playeridx .. tostring(i))
								...
								---race for taskbar	    
								UI_DisplayRace(SobGroup_OwnedBy("splitfighters"..playeridx .. tostring(i)))
								
								UI_DisplayExperience(SobGroup_OwnedBy("splitfighters"..playeridx .. tostring(i)), expFi1)
	  ...
	  --if hwsaving == 9 then
			...
			--Sun Damage
	              ...
	              DriveMode_DisplayButtonForShip("hgn_hscore"..z)
								...
								UI_DisplayRace(SobGroup_OwnedBy("hgn_hscore"..z))
								Temp_GlobalVariable.enablebtnparade[playerIndexList] = Temp_GlobalVariable.Selected_Save	    															
							end	  	        
		  ...
		  if (hwsaving == 3 and nco[playervar] >= 10) or nco[playervar] > 0 then			--nco[playerIndexList] < 10
			...
			--Sun Damage
		            ...
		            DriveMode_DisplayButtonForShip("splitcorvettes"..playeridx .. tostring(i))
								...
								---race for taskbar	    
								UI_DisplayRace(SobGroup_OwnedBy("splitcorvettes"..playeridx .. tostring(i)))
								
								UI_DisplayExperience(SobGroup_OwnedBy("splitcorvettes"..playeridx .. tostring(i)), expCo1)
	...
	--sonde  
	  ...
	  while playervar < Universe_PlayerCount() do
		...
		--Orders
					    ...
					    elseif SobGroup_AreAnyOfTheseTypes("splitothersnoparade"..playeridx .. tostring(i), "meg_drone_small, meg_drone_medium, meg_drone_large, meg_drone_scout") == 1 then
					    	if SobGroup_IsDoingAbility("splitothersnoparade"..playeridx .. tostring(i), AB_Guard) == 0 and 
					    	SobGroup_IsDoingAbility("splitothersnoparade"..playeridx .. tostring(i), AB_Attack) == 0 and 
					    	SobGroup_IsDoingAbility("splitothersnoparade"..playeridx .. tostring(i), AB_Move) == 0 then
					    		SobGroup_SetHealth("splitothersnoparade"..playeridx .. tostring(i), 0)
					    		for id, fx in {"Death","dmg01","dmg02","dmg03","Damage"} do
					    			FX_StopEvent("splitothersnoparade"..playeridx .. tostring(i), fx)
					    		end
					    	end
					    end
			          ...
			          DriveMode_DisplayButtonForShip("splitothersnoparade"..playeridx .. tostring(i))
								...
								---race for taskbar	    
								UI_DisplayRace(SobGroup_OwnedBy("splitothersnoparade"..playeridx .. tostring(i)))
	  ...
	  if (hwsaving == 8 and nf >= 10) or nf > 0 then			--nf < 10
		...
		--Orders                   
	            ...
	            DriveMode_DisplayButtonForShip("splitfrigates"..i)
						  ...
						  UI_DisplayExperience(SobGroup_OwnedBy("splitfrigates"..i), exp1)
						  
							UI_SetTextLabelText("NewTaskbar", "unitactualspeed", ""..floor(sqrt(SobGroup_GetActualSpeed("splitfrigates"..i))));
							---race for taskbar	    
							UI_DisplayRace(SobGroup_OwnedBy("splitfrigates"..i))
...
---HAXOR!!!!!-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
		...
		if UI_IsScreenActive("haxor") == 1 and GetGameRubric() ~= GR_MULTIPLAYER then
			...
			if UI_IsNamedElementVisible("haxor", "killselected") == 1 then
				local shipsNUM = PrepareSelectedShipsForHaxor("haxor_killselect"..playerIndex, 1)
				if shipsNUM > 0 then
					...
					--end
					local targetidentity = Haxor_GetTargetIdentity(playerIndex, "haxor_killselect"..playerIndex, {"O_____O! Own ", "Dafaq! Friendly ", "NICE HAX! Enemy "})
					if Player_IsAlive(playerIndex) == 1 then
				    Subtitle_AddAccordingToRace(playerIndex,"<c=fad610>"..targetidentity.."ships blown up: "..shipsNUM.."</c>",2.711)
				  else
				  	Subtitle_Add(Actor_Keeper,"<c=fad610>"..targetidentity.."ships blown up: "..shipsNUM.."</c>",2.711)
				  end
				else
			...
			if UI_IsNamedElementVisible("haxor", "healselected") == 1 then
				local shipsNUM = PrepareSelectedShipsForHaxor("haxor_healselect"..playerIndex, 1)
				if shipsNUM > 0 then
					...
					--end
					local targetidentity = Haxor_GetTargetIdentity(playerIndex, "haxor_healselect"..playerIndex, {"LONG LIVE HAX! Own ", "Oh the humanity! Friendly ", "What was that for? Enemy "})
					if Player_IsAlive(playerIndex) == 1 then
			...
			if UI_IsNamedElementVisible("haxor", "invincibleselected") == 1 then
				local shipsNUM = PrepareSelectedShipsForHaxor("haxor_invincibleselect"..playerIndex, 1)
				if shipsNUM > 0 then
					...
					--end
					local targetidentity = Haxor_GetTargetIdentity(playerIndex, "haxor_invincibleselect"..playerIndex, {"d(-_-)b Own ", "@__@! Friendly ", "o__o!! Enemy "})
					if Player_IsAlive(playerIndex) == 1 then
			...
			if UI_IsNamedElementVisible("haxor", "hyperspaceinselected") == 1 then
				local shipsNUM = PrepareSelectedShipsForHaxor("haxor_hyperspaceinselect"..playerIndex, 1)
				local hyperNUM = 0
				if shipsNUM > 0 then
					...
					for i = 0, numships - 1, 1 do
						if SobGroup_AreAllInRealSpace("SingleHaxorToHyperspace_SOB"..i) == 1 and SobGroup_IsDoingAbility("SingleHaxorToHyperspace_SOB"..i, AB_Hyperspace) == 0 and SobGroup_HealthPercentage("SingleHaxorToHyperspace_SOB"..i) > 0 then
					  	if SobGroup_OwnedBy("SingleHaxorToHyperspace_SOB"..i) >= 0 then
					...
					if hyperNUM > 0 then
						...
						local targetidentity = Haxor_GetTargetIdentity(playerIndex, "haxor_hyperspaceinselect"..playerIndex, {"(n__n) Own ", "(T__T) Friendly ", "(X__X) Enemy "})
						if Player_IsAlive(playerIndex) == 1 then
			...
			if UI_IsNamedElementVisible("haxor", "promoteselected") == 1 then
				local shipsNUM = PrepareSelectedShipsForHaxor("haxor_promoteselect"..playerIndex, 0)
				if shipsNUM > 0 then
				  ...
				  while playervar < Universe_PlayerCount() do
					...
					if shippromoted > 0 then
						...
						local targetidentity = Haxor_GetTargetIdentity(playerIndex, "haxor_promoteselect"..playerIndex, {"XP HAX! Own ", "Yoshi! Friendly ", "Hehehehehehe... Enemy "})
						if Player_IsAlive(playerIndex) == 1 then
			...
			--honour points
			UI_SetTextLabelText("haxor", "honourselectlabel", "+100")
			if UI_IsNamedElementVisible("haxor", "honourselected") == 1 then
				local addvalue = 100
				UI_PlaySound("SFX_cow")
				honorbonus[PlayerSelected+1] = honorbonus[PlayerSelected+1] + addvalue
				local targetidentity = "(.Y.) Enemy "
				if PlayerSelected == playerIndex then
					targetidentity = "(^o^) Own "
				elseif AreAllied(playerIndex, PlayerSelected) == 1 then
					targetidentity = "(n_n) Friendly "
				end
				if Player_IsAlive(playerIndex) == 1 then
			    Subtitle_AddAccordingToRace(playerIndex,"<c=fad610>"..targetidentity.."honour points increased by "..addvalue.."</c>",2.711)
			  else
			  	Subtitle_Add(Actor_Keeper,""..GetPlayerNameWithColour(PlayerSelected).."<c=fad610>'s honour points increased by "..buildbooster.."</c>",2.711)
			  end
				UI_SetElementVisible("haxor", "honourselected", 0)
			end
			--burn research
			local researchNUM = 0
			...
			if UI_IsNamedElementVisible("haxor", "transferselected") == 1 then
				--normal ships
				local shipsNUM = PrepareSelectedShipsForHaxor("haxor_transferselect"..playerIndex, 1)
				local transferNUM = 0
				if cantransfer == 1 then
					if shipsNUM > 0 then
						local targetidentity = Haxor_GetTargetIdentity(playerIndex, "haxor_transferselect"..playerIndex, {"@______@ Own ", "T______T Friendly ", "Mind control! Enemy "})
			...
			if UI_IsNamedElementVisible("haxor", "manualselected") == 1 then
				local shipsNUM = PrepareSelectedShipsForHaxor("haxor_manualselect"..playerIndex, 0)
				if shipsNUM > 0 then
...
function PrepareSelectedShipsForHaxor(ship, include_neutral)
	...
	if SobGroup_Empty("AllShips") == 0 then
		local playerIndex = 0
	  while playerIndex < Universe_PlayerCount() do
			local numships = SobGroup_SplitGroupReference("SingleHaxorToSelect_SOB", "Player_Ships"..playerIndex, "AllShips", SobGroup_Count("Player_Ships"..playerIndex))
			for i = 0, numships - 1, 1 do
	...
	if include_neutral == 1 then
		...
		elseif SobGroup_Empty("splitter") == 0 then
			Player_FillProximitySobGroup("all_megalithnoowner", -1, "splitter", 10000000)
		end
		...
		local nummegaliths = SobGroup_SplitGroupReference("SingleMegalithSelected_SOB", "all_megalithnoowner", "splitter", SobGroup_Count("all_megalithnoowner"))
		if nummegaliths > 0 then
			for i = 0, nummegaliths - 1, 1 do
				if SobGroup_Selected("SingleMegalithSelected_SOB"..i) == 1 then
			    SobGroup_SobGroupAdd(ship, "SingleMegalithSelected_SOB"..i)
			  end
			end
		end
	end
...
function Haxor_GetTargetIdentity(playerIndex, ship, t)
	local targetidentity = "Unclaimed "
	if SobGroup_OwnedBy(ship) >= 0 then
		if playerIndex == SobGroup_OwnedBy(ship) then
			targetidentity = t[1] --"Own "
		elseif AreAllied(playerIndex, SobGroup_OwnedBy(ship)) == 1 then
			targetidentity = t[2] --"Friendly "
		else
			targetidentity = t[3] --"Enemy "
		end
	end
...
--tactics
  ...
  elseif ordine == "dock" then    
    --if SobGroup_GetHardPointHealth(ship, "hgn_orders_dock") > 0 then	             
      --if SobGroup_FillProximitySobGroup ("temp", "parader"..SobGroup_OwnedBy(ship), ship, 7000) == 1 then           
	      --SobGroup_DockSobGroup_Smart( "temp", ship)				                  
	    --end 
	    --SobGroup_SetHardPointHealth(ship, "hgn_orders_dock", 0)	 
    --end
    if SobGroup_GetHardPointHealth(ship, "hgn_orders_dock") > 0 then
  ...
  elseif ordine == "dock_permanent" then    
    if SobGroup_GetHardPointHealth(ship, "hgn_orders_dock_permanent") > 0 then	             
      if SobGroup_FillProximitySobGroup ("temp", "parader"..SobGroup_OwnedBy(ship), ship, 7000) == 1 then           
	      SobGroup_DockSobGroupAndStayDocked_Smart( "temp", ship)				                  
	    end 
  ...
  elseif ordine == "gujkingdisable" then    
    if SobGroup_GetHardPointHealth(ship, "hgn_orders_gujkingdisable") > 0 then 	 
      if SobGroup_IsDockedSobGroup("hgn_mothership"..SobGroup_OwnedBy(ship), ship) == 1 then
        SobGroup_Launch_Smart("hgn_mothership"..SobGroup_OwnedBy(ship), ship)
      end  
      if SobGroup_IsDockedSobGroup("hgn_power"..SobGroup_OwnedBy(ship), ship) == 1 then
        SobGroup_Launch_Smart("hgn_power"..SobGroup_OwnedBy(ship), ship)
      end 
      if SobGroup_IsDockedSobGroup("hgn_researchstation"..SobGroup_OwnedBy(ship), ship) == 1 then
        SobGroup_Launch_Smart("hgn_researchstation"..SobGroup_OwnedBy(ship), ship)
      end 
      if SobGroup_IsDockedSobGroup("hgn_crewstation"..SobGroup_OwnedBy(ship), ship) == 1 then
        SobGroup_Launch_Smart("hgn_crewstation"..SobGroup_OwnedBy(ship), ship)
      end 
      if SobGroup_IsDockedSobGroup("hgn_combatbase"..SobGroup_OwnedBy(ship), ship) == 1 then
        SobGroup_Launch_Smart("hgn_combatbase"..SobGroup_OwnedBy(ship), ship)
      end   
  ...
  elseif ordine == "evacuation" then    
    ...
    if SobGroup_Empty("temp1") == 0 then
      ...
      SobGroup_SetHardPointHealth("temp1", "pilot", numpilotstosave/100) 
	    ...
	    SobGroup_Launch_Smart("temp1", ship)
    end
    if SobGroup_GetHardPointHealth(ship, "hgn_orders_evacuation") > 0 then    	
    	--disable
      ...
      SobGroup_Launch_Smart("temp", ship)  
    end
  ...
  elseif ordine == "unplug" then
  	...
  	if SobGroup_GetHardPointHealth(ship, "hgn_orders_unplug") > 0 then
  		if generatordocked == 1 then
  			SobGroup_Launch_Smart("KprGeneratorsToLaunch", ship)
...
--tactical reinforcements  
  elseif ordine == "tactical" then    
		ProcessTacticalOrders(ship)
  end 	--of order types
				        ...
				        SobGroup_AreAllInRealSpace("SingleAutodockAlliedFiCo_SOB"..z)==1 then
				        	...
				        	if SobGroup_FillProximitySobGroup("temp", "temp_acceptingfcdock"..playerIndex, "SingleAutodockAlliedFiCo_SOB"..z, d) == 1 then
					        	...
					        	local numdocks = SobGroup_SplitGroupReference("SingleOrderAutoDock_SOB", "temp", "Player_Ships"..playerIndex, SobGroup_Count("temp"))
								    ...
								    if can_dock == 1 then
								    	SobGroup_DockSobGroup_Smart("SingleAutodockAlliedFiCo_SOB"..z, "temp_orderdocktartet")
								    end
								  end
			        	end
			        end
				  	...
				  	if SobGroup_GetHardPointHealth("hgn_shipyard_g"..i, "msdocker") > 0 and
				        ...
				        elseif Player_HasResearch(SobGroup_OwnedBy("hgn_shipyard_g"..i), "ShipyardMAXSHIELDUpgrade1") == 1 then
					      ...
					      if bloccoshield[SobGroup_OwnedBy("hgn_shipyard_g"..i)+1] == 0 then
					        SobGroup_Launch_Smart("hgn_massshield"..SobGroup_OwnedBy("hgn_shipyard_g"..i), "hgn_shipyard_g"..i)					        
...
function UI_DisplayRace(player)
	if player >= 0 then	--normal players
	...
	else	--megaliths or whatever
		UI_SetTextLabelText("NewTaskbar", "unitrace", "???");
	end
end

function UI_DisplayExperience(player, experience)
	UI_SetTextLabelText("NewTaskbar", "experience_title", "    "..Experience_GetTitle(player, experience));
	UI_SetTextLabelText("NewTaskbar", "experience", " ("..floor(experience).." XP)");
	---medals
  if experience >= 999 then
    UI_SetElementVisible("NewTaskbar", "medal10", 1)
    UI_SetElementSize("NewTaskbar", "medal5", 0, 0)
...
function UI_ShowSingleElementInList(screen, element, list)	--show one and hide all others (for event pictures etc.)
	for z, iCount in list do
		if element==iCount then
			UI_SetElementVisible(screen, iCount, 1)
		else
			UI_SetElementVisible(screen, iCount, 0)
		end
	end
end
...
function UI_ShowRandomElement(screen, element, enable)
	...
	elseif screen == "general" then
		num = 8
	elseif screen == "crisis" then
		num = 23
	elseif screen == "easteregg" then
		if element == "btnOk" then
			num = 1
		elseif element == "btnNewYear" then
			num = 2
		elseif element == "btnChristmas" then
			num = 4
		end
	end
	for x=1,num do
...
function UI_ShowRandomButtonWithDialogue(playerIndex, screen)
	...
	UI_SetElementVisible(screen, prefix..ButtonTable[button_type][1]..random(1,ButtonTable[button_type][2]), 1)
end