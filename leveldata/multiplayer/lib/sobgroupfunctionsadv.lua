...
function SobGroup_CreateShip_InstantSpawnDock(builder, shipname, override_mode)	--spawn a ship then instantly dock it to the builder (and launch)
	...
	SobGroup_IsDocked(single_builder) == 0 then
		...
		AI_SobGroup_SpawnNewShipInSobGroup(playerIndex, shipname, "InstantSpawn", "SOB_InstantSpawnDock", VOL)
		Volume_Delete(VOL)
		...
		String_ContainsPatternInTable(strlower(single_builder), no_dockpath_single_builders) == 0 then	--spawn docked then launch
		...
		elseif override_mode == 1 then	--just instant parade
		...
		elseif override_mode == 2 and
		SobGroup_CanDoAbility("SOB_InstantSpawnDock", AB_Dock) == 1 and 
		String_ContainsPatternInTable(strlower(shipname), undockable_ships) == 0 and 
		String_ContainsPatternInTable(strlower(single_builder), no_dockpath_single_builders) == 0 then	--stay docked
			--print("Spawn-docking "..shipname.." from "..single_builder.." owned by player index: "..playerIndex..", game year: "..year)
			SobGroup_DockSobGroupAndStayDocked("SOB_InstantSpawnDock", single_builder)
			SobGroup_DockSobGroupInstant("SOB_InstantSpawnDock", single_builder)
		elseif override_mode == 3 then	--no parade
			--print("Spawning "..shipname.." at "..single_builder.." owned by player index: "..playerIndex..", game year: "..year)
		end
		ship_created = 1
	end
	return ship_created
...
function SobGroup_CreateShip_InstantSpawnStayDocked(builder, shipname)
	SobGroup_CreateShip_InstantSpawnDock(builder, shipname, 2)
end

function SobGroup_CreateShip_InstantSpawnAt(builder, shipname)
	SobGroup_CreateShip_InstantSpawnDock(builder, shipname, 3)
end

function SobGroup_Attack_Smart(playerIndex, attacker, target)		--order all enemies of playerIndex to attack target; this might cause 00000000 crash
...
function SobGroup_AttractSobGroupInEnemyGroup(ship, enemy_group, subgroup)	--order a specific type of enemy to attack the ship
	SobGroup_Create("TempCompareSOB")
	SobGroup_Clear("TempCompareSOB")
	SobGroup_FillCompare("TempCompareSOB", enemy_group, subgroup)
	local numships = SobGroup_SplitGroupReference("SingleEnemySOB", "TempCompareSOB", ship, SobGroup_Count("TempCompareSOB"))
	for counter = 0, numships - 1, 1 do
		if SobGroup_CanDoAbility("SingleEnemySOB"..counter, AB_Attack) == 1 and 
		SobGroup_AreAllInRealSpace("SingleEnemySOB"..counter) == 1 then
			SobGroup_Attack(max(0,SobGroup_OwnedBy("SingleEnemySOB"..counter)), "SingleEnemySOB"..counter, ship)
		end
	end
	SobGroup_Clear("TempCompareSOB")
end

function SobGroup_IsAttackingTargetWithinDistance(ship, enemy_SOB, distance, strict_criteria)
	local strict_mode = 0
	if strict_criteria ~= nil then
		strict_mode = strict_criteria
	end
	if SobGroup_Empty(ship) == 0 and SobGroup_Empty(enemy_SOB) == 0 then
		if SobGroup_IsDoingAbility(ship, AB_Attack) == 1 then
			if strict_mode == 1 then
				if SobGroup_UnderAttack(enemy_SOB) == 1 then
					local numships = SobGroup_SplitGroupReference("SingleEnemySOB", enemy_SOB, ship, SobGroup_Count(enemy_SOB))
					for counter = 0, numships - 1, 1 do
						SobGroup_Create("SOB_TempAttackers")
						SobGroup_Clear("SOB_TempAttackers")
						SobGroup_GetAttackers("SingleEnemySOB"..counter, "SOB_TempAttackers")
						if SobGroup_SobGroupCompare(ship, "SOB_TempAttackers") == 1 then
							if SobGroup_GetDistanceToSobGroup(ship, "SOB_TempAttackers") < distance then
								return 1
							end
						end
						SobGroup_Clear("SOB_TempAttackers")
				...
				if SobGroup_IsGroupNearybyExcludingSelf(ship, enemy_SOB, distance) == 1 then
					return 1
				end
			end
		end
...
function Table_Identical(TableA, TableB)
	if getn(TableA) ~= getn(TableB) then
		return 0
	else
		for z, iCount in TableA do
			if iCount ~= TableB[z] then
				return 0
			end
		end
	end
      ...
      --reassurance! VERY IMPORTANT!     
		  if CPU_Exist_Smart(playerIndex) == 1 and GetGameRubric() ~= GR_CAMPAIGN then
		    if Player_CanResearch(playerIndex, "isai" ) == 1 then
		 	...
		 	--gradually restore AI for ships spawned without AI (00000000 crash solution)
		 	if CPU_Exist_Smart(playerIndex) == 1 then
		 		AI_RestoreAIAfterSpawn(playerIndex)
		 	end
...
function keeper3Sek()	
	if Temp_GlobalVariable.DroneSave > DroneTechnolgy_Runtimes then
		Temp_GlobalVariable.DroneSave = 1
	end
	...
	while (playerIndex < playerCount) do
		...
		--drone tech
		if Player_HasResearch(playerIndex, "DroneTechBattleDrone3") == 1 then
			DroneTechnolgy(playerIndex, "meg_drone_large")
		elseif Player_HasResearch(playerIndex, "DroneTechBattleDrone2") == 1 then
			DroneTechnolgy(playerIndex, "meg_drone_medium")
		elseif Player_HasResearch(playerIndex, "DroneTechBattleDrone1") == 1 then
			DroneTechnolgy(playerIndex, "meg_drone_small")
		end
		if Player_HasResearch(playerIndex, "DroneTechScoutDrone1") == 1 then
			DroneTechnolgy(playerIndex, "meg_drone_scout")
		end
		
		playerIndex = playerIndex + 1
	end
	
	Temp_GlobalVariable.DroneSave = Temp_GlobalVariable.DroneSave + 1
						...
						if SobGroup_Empty("Reaver_Confused_SOB") == 0 then
							...
							for z = 0, numplasma -1, 1 do
								if SobGroup_Empty("Reaver_FeedbackTarget_SOB_Single"..z) == 0 and Player_GetNumberOfSquadronsOfTypeAwakeOrSleeping(playerIndex, "kpr_lightgranule_tiny") < 50 then
									...
									if CPU_Exist_Smart(playerIndex) == 1 then
										CPU_RemoveSobGroup(playerIndex, "Reaver_SinglePlasmaBall_TempSOB")
									end
...
function phaseBlowoutKeeper(playerIndex)
	...
	if numsleepers > 0 then
		...
		for counter = 0, numsleepers -1, 1 do
			if SobGroup_HealthPercentage("DreadSingle_SOB"..counter) > 0 then
				if SobGroup_IsDoingAbility("DreadSingle_SOB"..counter, AB_DefenseField) == 1 then
					SobGroup_SetHealth_Smart("DreadSingle_SOB"..counter, SobGroup_HealthPercentage("DreadSingle_SOB"..counter) + 0.15)
				end
			end
			...
			for counter = 0, numsquadrons -1, 1 do
				...
				SobGroup_AreAllInRealSpace("ShipBuilding"..counter) == 1 then
					...
					elseif (SobGroup_HealthPercentage("ShipBuilding".. counter) > 0.99) then		--==1
						...
						AI_SobGroup_SpawnNewShipInSobGroup(playerIndex, createtype, "Currentbuild", "Currentbuild", "Buildvolume")
...
DroneTechnolgy_Runtimes = 6
function DroneTechnolgy(playerIndex, drone_type)
	if Temp_GlobalVariable.DroneSave == 1 then
		for i = 0,nms-1,1 do
			local ship = "split".."motherships"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1500)
			end
		end
		for i = 0,nd-1,1 do
			local ship = "split".."destroyers"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,nsy-1,1 do
			local ship = "split".."shipyards"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1500)
			end
		end
	elseif Temp_GlobalVariable.DroneSave == 2 then
		for i = 0,nc-1,1 do
			local ship = "split".."carriers"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,nbc-1,1 do
			local ship = "split".."battlecruisers"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1200)
			end
		end
		for i = 0,nj-1,1 do
			local ship = "split".."juggernaughts"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
	elseif Temp_GlobalVariable.DroneSave == 3 then
		for i = 0,nf-1,1 do
			local ship = "split".."frigates"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
	elseif Temp_GlobalVariable.DroneSave == 4 then
		for i = 0,nmc-1,1 do
			local ship = "split".."minecontainers"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,ntrc-1,1 do
			local ship = "split".."containers"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,nmb-1,1 do
			local ship = "split".."miningbases"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 2500)
			end
		end
	elseif Temp_GlobalVariable.DroneSave == 5 then
		for i = 0,nrs-1,1 do
			local ship = "split".."researchstations"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,nps-1,1 do
			local ship = "split".."powerstations"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,nws-1,1 do
			local ship = "split".."weaponstations"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,ncs-1,1 do
			local ship = "split".."crewstations"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,ncf-1,1 do
			local ship = "split".."commandfortresses"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 2000)
			end
		end
		for i = 0,nmr-1,1 do
			local ship = "split".."mobilerefineries"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,nrc-1,1 do
			local ship = "split".."resourcecollectors"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
	elseif Temp_GlobalVariable.DroneSave == 6 then
		for i = 0,nhs-1,1 do
			local ship = "split".."hyperspacestations"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
		for i = 0,np-1,1 do
			local ship = "split".."platforms"..i
			if SobGroup_OwnedBy(ship) == playerIndex then
				DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, 1000)
			end
		end
...
function DroneTechnolgy_LaunchDrone(playerIndex, ship, drone_type, radius)
	if SobGroup_HealthPercentage(ship) > 0 and 
	(SobGroup_IsDoingAbility(ship, AB_Attack) == 1 or SobGroup_UnderAttack(ship) == 1) and 
	SobGroup_AreAllInRealSpace(ship) == 1 and 
	SobGroup_IsDoingAbility(ship, AB_Dock) == 0 and 
	SobGroup_IsDoingAbility(ship, AB_Retire) == 0 and 
	SobGroup_IsDoingAbility(ship, AB_Hyperspace) == 0 and 
	SobGroup_IsDoingAbility(ship, AB_HyperspaceViaGate) == 0 and 
	sqrt(SobGroup_GetSpeed(ship)) < 400 and 
	SobGroup_IsGroupNearybyExcludingSelf(ship, drone_type..playerIndex, radius) == 0 then
		local VOL = "VOL_SOB_DroneSpawn"..playerIndex.."_"..ship
		Volume_AddSphere(VOL, SobGroup_GetPosition(ship), 6000)
		SobGroup_Create("SOB_DroneSpawn")
		SobGroup_Clear("SOB_DroneSpawn")
		SobGroup_SpawnNewShipInSobGroup(playerIndex, drone_type, "InstantSpawn", "SOB_DroneSpawn", VOL)
		Volume_Delete(VOL)
		SobGroup_GuardSobGroup("SOB_DroneSpawn", ship)
--		SobGroup_MoveToSobGroup("SOB_DroneSpawn", ship)
--		SobGroup_SetTactics_Smart("SOB_DroneSpawn", AggressiveTactics)
	end
end
		...
		if SobGroup_HealthPercentage("SingleUpgradeHgnDestroyer_SOB".. counter) > 0 then
	        ...
	        theta = random_pseudo(-90,90)
					...
					random_pseudo(0, 1+numships) < 5 then	--anti-lag
						...
						SobGroup_AttractSobGroupInEnemyGroup("SOB_HgnDummyECM", "Temp_EnemySob"..playerIndex, "nuclearbombs")
						SobGroup_Clear("Temp_EnemySob"..playerIndex)
					end
					...
					if random_pseudo(0, 1+SobGroup_Count("hgn_ALLdestroyers"..playerIndex)) < 5 then	--anti-lag
						if SobGroup_FillProximitySobGroupExceptSelf("SOB_ECMenemyGroup", "enemies"..playerIndex, dummy, radius) == 1 then
							...
							if SobGroup_Empty("SOB_ECMenemyGroup") == 0 then
								...
								SobGroup_AttractSobGroupInEnemyGroup(dummy, "SOB_ECMenemyGroup", "nuclearbombs")
	...
	for counter = 0, numships - 1, 1 do
		if SobGroup_HealthPercentage("SingleUpgradeVgrDestroyer_SOB".. counter) > 0 then
	        ...
	        theta = random_pseudo(-90,90)
					...
					if CPU_Exist_Smart(playerIndex) == 1 then
						CPU_RemoveSobGroup(playerIndex, "SOB_VgrDummySmoke")
					end
					...
					random_pseudo(0, 1+numships) < 5 then	--anti-lag
						...
						SobGroup_AttractSobGroupInEnemyGroup("SOB_VgrDummySmoke", "Temp_EnemySob"..playerIndex, "nuclearbombs")
						SobGroup_Clear("Temp_EnemySob"..playerIndex)
					end
					if Player_HasResearch(playerIndex, "CapitalSmokeScreen1" ) == 1 then
						if playerIndex ~= Universe_CurrentPlayer() and AreAllied(playerIndex, Universe_CurrentPlayer()) == 0 then
					...
					if random_pseudo(0, 1+SobGroup_Count("vgr_ALLsmokers"..playerIndex)) < 5 then	--anti-lag
						if SobGroup_FillProximitySobGroupExceptSelf("SOB_SmokeEnemyGroup", "enemies"..playerIndex, dummy, radius) == 1 then
							...
							if SobGroup_Empty("SOB_SmokeEnemyGroup") == 0 then
								...
								SobGroup_AttractSobGroupInEnemyGroup(dummy, "SOB_SmokeEnemyGroup", "nuclearbombs")
...
--- XP ability main function
			...
			for i = 1, random_pseudo(1,3) do
				if Player_HasResearch(playerIndex, "SpecialAbility1Level3" ) == 1 then
					...
					i == 1 then	--anti-lag
						...
						SobGroup_AttractSobGroupInEnemyGroup("SmokeGrenadeSOB_"..ship, "Temp_EnemySob"..playerIndex, "nuclearbombs")
						SobGroup_Clear("Temp_EnemySob"..playerIndex)
					end
				...
				if CPU_Exist_Smart(playerIndex) == 1 then
					CPU_RemoveSobGroup(playerIndex, "SmokeGrenadeSOB_"..ship)
				end
			end
		...
		if SobGroup_GetHardPointHealth(ship, "rad_xp_pointdefense") > 0 then
			...
			for i = 1, random_pseudo(1,quantity) do
				...
				if CPU_Exist_Smart(playerIndex) == 1 then
					CPU_RemoveSobGroup(playerIndex, "pdsDroneSOB_"..ship.."_"..BigSerial)
				end
		...
		if SobGroup_GetHardPointHealth(ship, "rad_xp_ambush") > 0 then
			...
			if NeededPilots[playerIndex + 1] >= spam_num * 6 then
				...
				for i = 1, spam_num do
					...
					AI_SobGroup_SpawnNewShipInSobGroup(SobGroup_OwnedBy(ship), Radboostships[shiptype], "AmbushSummonSOB_"..ship, "AmbushSummonSOB_"..ship, "ambushVOL_"..ship)
				end
				if Player_HasResearch(playerIndex, "SpecialAbility4Level2" ) == 1 then
					AI_SobGroup_SpawnNewShipInSobGroup(SobGroup_OwnedBy(ship), RadboostshipsAlt[random_pseudo(1, Table_CountElements(RadboostshipsAlt))], "AmbushSummonSOB_"..ship, "AmbushSummonSOB_"..ship, "ambushVOL_"..ship)
				end
				if Player_HasResearch(playerIndex, "SpecialAbility4Level3" ) == 1 then
					AI_SobGroup_SpawnNewShipInSobGroup(SobGroup_OwnedBy(ship), RadAllfrigates[random_pseudo(1, Table_CountElements(RadAllfrigates))], "AmbushSummonSOB_"..ship, "AmbushSummonSOB_"..ship, "ambushVOL_"..ship)
				end
		...
		if SobGroup_GetHardPointHealth(ship, "rad_xp_nuke") > 0 then
			...
			if CPU_Exist_Smart(playerIndex) == 1 then
				CPU_RemoveSobGroup(playerIndex, "dirtynukeSOB_"..ship)
			end
	...
	if Player_GetRace(playerIndex) == Race_Keeper then
		...
		if SobGroup_GetHardPointHealth(ship, "kpr_ab_inertia") > 0 then
			...
			if Player_HasResearch(playerIndex, "SpecialPowerInertiaDrive3") == 1 then		--FTL drive
				...
				if SobGroup_GetHardPointHealth(ship, "kpr_ab_inertia") > 0.951 then		--prevent spawning multiple dummy objects
					...
					if CPU_Exist_Smart(playerIndex) == 1 then
						CPU_RemoveSobGroup(playerIndex, "SOB_KprDummyRepulsor")
					end
		...
		if SobGroup_GetHardPointHealth(ship, "kpr_ab_am") > 0 then
			...
			if SobGroup_GetHardPointHealth(ship, "kpr_ab_am") > 0.901 then
				...
				if CPU_Exist_Smart(playerIndex) == 1 then
					CPU_RemoveSobGroup(playerIndex, "SOB_KprDummyAM")
				end
				local y = 0
		...
		if SobGroup_GetHardPointHealth(ship, "kpr_ab_blackhole") > 0 then
			...
			if SobGroup_GetHardPointHealth(ship, "kpr_ab_blackhole") > 0.991 then
				...
				if CPU_Exist_Smart(playerIndex) == 1 then
					CPU_RemoveSobGroup(playerIndex, "SOB_KprDummyBlackhole")
				end
		...
		if SobGroup_GetHardPointHealth(ship, "kpr_ab_particle") > 0 then
			...
			local shot=1
			if Player_HasResearch(playerIndex, "SpecialPowerLightGranule2") == 1 then	--second shot
				shot = 2
			end
			for i=1,shot,1 do
				SobGroup_SpawnNewShipInSobGroup(playerIndex, "kpr_lightgranule", "kpr_Light", "SOB_KprDummyLight", "VOL_KprDummyLight"..currenttime)
				if CPU_Exist_Smart(playerIndex) == 1 then
					CPU_RemoveSobGroup(playerIndex, "SOB_KprDummyLight")
				end
			end
