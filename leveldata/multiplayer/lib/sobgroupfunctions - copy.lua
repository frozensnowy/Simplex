...
---megalith/battlearena
  SobGroup_Create ("megalithnoowner")
	...
	SobGroup_SetSwitchOwnerFlag("all_megalithnoowner", 0)
...
--Shipwreck
	...
	for z, iCount in ship_list.hgn do
		if iCount.WreckSpawnTime ~= nil then
			t = t + 1
			wreck_list.shipname[t] = iCount.name
			wreck_list.spawntime[t] = iCount.WreckSpawnTime
		end
	end
	...
	print("Shipwreck system initialized! Types of ships capable of spawning wreck: "..t)
--AI hyperspace warfare
...
--SplitGroup    
  ...
  if SobGroup_Empty("splitter") == 1 then
  	Volume_AddSphere("VOL_SplitterAtCenter", {100+random(-1000,1000),21000+random(-1000,1000),100+random(-1000,1000)}, 1000)
...
---Group > LastGroup
    ...
    if SobGroup_Count("temp") == 1 then    
	    ...
	    SobGroup_SobGroupAdd("split" .. tostring(Group) .. tostring(Counter), "temp")	
	...
	--name	    
	    if HasName == 1 then 
	      if SobGroup_OwnedBy("split" .. tostring(Group) .. tostring(Counter)) == Universe_CurrentPlayer() then			  
		      ...
		      elseif SobGroup_AreAnyOfTheseTypes("split" .. tostring(Group) .. tostring(Counter), "hgn_shipyard") == 1 then   
		      else
		        RenameAccordingToDiffentRaces("split" .. tostring(Group) .. tostring(Counter))
  ...
  --caso dei container	      
	    if SobGroup_AreAnyOfTheseTypes("split" .. tostring(Group) .. tostring(Counter), "hgn_minecontainer, vgr_minecontainer") == 1 then
  ...
  ---se > di 1	      
	    ...
	    --per navi soggette ad essere spesso nella zona launchtimebeforestart
      --da fare
	...
	--x cs ricostrzine moduli se distrutta
	    if SobGroup_AreAnyOfTheseTypes("split" .. tostring(Group) .. tostring(Counter), "hgn_crewstation") == 1 then   
	      ...
	      if SobGroup_GetHardPointHealth("supporter"..SobGroup_OwnedBy("split" .. tostring(Group) .. tostring(Counter)), "isLieutenant") > 0 and gradocorrente >= 1 then
	        SobGroup_CreateSubSystem("split" .. tostring(Group) .. tostring(Counter), "isLieutenantCore")     
	      end  
	      
	      if SobGroup_GetHardPointHealth("supporter"..SobGroup_OwnedBy("split" .. tostring(Group) .. tostring(Counter)), "isCommander") > 0 and gradocorrente >= 2 then
	        SobGroup_CreateSubSystem("split" .. tostring(Group) .. tostring(Counter), "isCommanderCore")  	      
	      end  
	      
	      if SobGroup_GetHardPointHealth("supporter"..SobGroup_OwnedBy("split" .. tostring(Group) .. tostring(Counter)), "isCaptain") > 0 and gradocorrente >= 3 then
	        SobGroup_CreateSubSystem("split" .. tostring(Group) .. tostring(Counter), "isCaptainCore") 	       
	      end  
	      
	      if SobGroup_GetHardPointHealth("supporter"..SobGroup_OwnedBy("split" .. tostring(Group) .. tostring(Counter)), "isCommodore") > 0 and gradocorrente >= 4 then
	        SobGroup_CreateSubSystem("split" .. tostring(Group) .. tostring(Counter), "isCommodoreCore") 	      
	      end  
	      
	      if SobGroup_GetHardPointHealth("supporter"..SobGroup_OwnedBy("split" .. tostring(Group) .. tostring(Counter)), "isAdmiral") > 0 and gradocorrente >= 5 then
	        SobGroup_CreateSubSystem("split" .. tostring(Group) .. tostring(Counter), "isAdmiralCore")  	          
	      end  
	      
	      if SobGroup_GetHardPointHealth("supporter"..SobGroup_OwnedBy("split" .. tostring(Group) .. tostring(Counter)), "isFleetAdmiral") > 0 and gradocorrente >= 6 then
	        SobGroup_CreateSubSystem("split" .. tostring(Group) .. tostring(Counter), "isFleetAdmiralCore") 	      
	      end  
	    end      	        
	--FX
			  if SobGroup_AreAnyOfTheseTypes("split" .. tostring(Group) .. tostring(Counter), "hgn_hyperspacestation") == 1 then    
...
---Group < Counter    
  elseif SobGroup_Count(Group) < Counter then         
...
function SobGroup_SplitGroupReference(SobGroupOut, SobGroupToSplit, ReferenceSobGroup, NumberToSplit)
    -- function created by Apollyon470		--advanced version (COMPLEX SIMPLE)
